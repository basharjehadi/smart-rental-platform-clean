...
describe('LeaseEventService', () => {
  ...
  beforeEach(() => {
    ...
    badgeService.createBadge.mockResolvedValue({ id: 'badge-1', name: 'EARLY_TERMINATION_HANDLER' });
  ...
  describe('handleLeaseStatusChange', () => {
    test('should handle lease ended status', async () => {
      ...
      await leaseEventService.handleLeaseStatusChange('lease-1', 'ACTIVE', 'ENDED', metadata);
    ...
    test('should handle TERMINATED_24H status', async () => {
      ...
      await leaseEventService.handleLeaseStatusChange('lease-1', 'ACTIVE', 'TERMINATED_24H', metadata);
    ...
    test('should handle TERMINATED status', async () => {
      ...
      await leaseEventService.handleLeaseStatusChange('lease-1', 'ACTIVE', 'TERMINATED', metadata);

      expect(prisma.lease.findUnique).toHaveBeenCalled();
    });

    test('should handle unknown status gracefully', async () => {
      await leaseEventService.handleLeaseStatusChange('lease-1', 'ACTIVE', 'UNKNOWN_STATUS', {});
    ...
    test('should handle lease not found gracefully', async () => {
      ...
      await expect(
        leaseEventService.handleLeaseStatusChange('nonexistent-lease', 'ACTIVE', 'ENDED', {})
  ...
  describe('handleLeaseTerminated24H', () => {
    ...
    test('should award early termination badge to tenant', async () => {
      ...
      expect(badgeService.awardBadgeToUser).toHaveBeenCalledWith('tenant-1', 'badge-1');
    });
  ...
  describe('createEndOfLeaseReviews', () => {
    ...
    test('should handle missing tenant group gracefully', async () => {
      ...
      const options = { isEarlyTermination: false, excludeFromAggregates: false };

      await expect(
        leaseEventService.createEndOfLeaseReviews(leaseWithoutTenantGroup, options)
      ).rejects.toThrow();
    });
  ...
  describe('processLeaseStatusChecks', () => {
    ...
    test('should handle errors gracefully and continue processing', async () => {
      ...
      await expect(leaseEventService.processLeaseStatusChecks()).resolves.toBeUndefined();
    });
  });
});
