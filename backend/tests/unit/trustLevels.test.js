...
describe('Trust Levels Service', () => {
  ...
  describe('tenantTrustLevel', () => {
    test('should return New level for user with no reviews', async () => {
      ...
      const result = await trustLevelService.tenantTrustLevel('user1', mockPrisma);
    ...
    test('should return Trusted level for user with 10+ reviews, 4.2+ rating, 95%+ on-time', async () => {
      ...
      expect(result.reasons).toContain('Excellent on-time payment rate (100.0%)');
    ...
    test('should return New level when below Reliable thresholds', async () => {
      ...
      expect(result.reasons).toContain('On-time rate below threshold (50.0%/80%)');
    });
    ...
    test('should handle edge case with no payments', async () => {
      ...
      expect(result.level).toBe('New');
      expect(result.reasons).toContain('On-time rate below threshold (0.0%/80%)');
  ...
  describe('landlordTrustLevel', () => {
    test('should return New level for user with no reviews', async () => {
      ...
      const result = await trustLevelService.landlordTrustLevel('user1', mockPrisma);
    ...
    test('should return New level when below Reliable thresholds', async () => {
      ...
      expect(result.level).toBe('New');
      expect(result.reasons).toContain('Insufficient reviews (2/3)');
      expect(result.reasons).toContain('Accuracy rate below threshold (0.0%/80%)');
    });
  ...
  describe('getUserTrustLevel', () => {
    test('should return tenant trust level for user with only tenant history', async () => {
      ...
      trustLevelService.tenantTrustLevel = jest.fn().mockResolvedValue(mockTenantResult);

      const result = await trustLevelService.getUserTrustLevel('user1', mockPrisma);

      expect(result).toEqual(mockTenantResult);
      expect(trustLevelService.tenantTrustLevel).toHaveBeenCalledWith('user1', mockPrisma);
    ...
    test('should return landlord trust level for user with only landlord history', async () => {
      ...
      trustLevelService.landlordTrustLevel = jest.fn().mockResolvedValue(mockLandlordResult);

      const result = await trustLevelService.getUserTrustLevel('user1', mockPrisma);

      expect(result).toEqual(mockLandlordResult);
      expect(trustLevelService.landlordTrustLevel).toHaveBeenCalledWith('user1', mockPrisma);
  ...
  describe('Error handling', () => {
    test('should handle database errors gracefully', async () => {
      mockPrisma.review.findMany.mockRejectedValue(new Error('Database connection failed'));

      await expect(trustLevelService.tenantTrustLevel('user1', mockPrisma))
        .rejects
        .toThrow('Failed to calculate tenant trust level');
    });
    ...
    test('should handle missing payment data gracefully', async () => {
      ...
      mockPrisma.review.findMany.mockResolvedValue(reviews);
      mockPrisma.payment.findMany.mockRejectedValue(new Error('Payment table not found'));
      mockPrisma.dispute.count.mockResolvedValue(0);
