...
async function debugPropertyType() {
  try {
    ...
    console.log(`🏠 Rental Request Property Type: "${rentalRequest.propertyType}"`);
    ...
    console.log(`1. Exact match: ${exactMatch} ("${rentalRequest.propertyType}" === "${property.propertyType}")`);

    // Test 2: Case-insensitive match
    const caseInsensitiveMatch = rentalRequest.propertyType.toLowerCase() === property.propertyType.toLowerCase();
    console.log(`2. Case-insensitive match: ${caseInsensitiveMatch} ("${rentalRequest.propertyType.toLowerCase()}" === "${property.propertyType.toLowerCase()}")`);
    ...
    console.log(`3. Prisma query with exact propertyType("${property.propertyType}"): ${testQuery1.length} results`);

    // Test 4: Prisma query with case-insensitive (this won't work in Prisma)
    console.log(`4. Note: Prisma doesn't support case-insensitive queries by default`);
    ...
    console.log(`8. propertyType = "${property.propertyType}": ${propertyTypeQuery.length} results`);
    ...
    console.log(`10. All property types in properties: ${allPropertyPropertyTypes.map(p => `"${p.propertyType}"`).join(', ')}`);
