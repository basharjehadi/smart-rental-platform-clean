...
async function migrateToOrganizationModel() {
  console.log('🚀 Starting migration to Organization and TenantGroup models...');
  ...
  try {
    ...
    for (const landlord of landlords) {
      ...
      console.log(`✅ Added ${landlord.name || landlord.id} as OWNER of organization`);
    }
    ...
    for (const tenant of tenants) {
      ...
      console.log(`✅ Created tenant group: ${tenantGroup.name} (${tenantGroup.id})`);
      ...
      console.log(`✅ Added ${tenant.name || tenant.id} as PRIMARY member of tenant group`);
    }
    ...
    for (const property of properties) {
      if (property.landlordId) {
        ...
        if (orgMember) {
          ...
          console.log(`✅ Updated property ${property.id} to reference organization ${orgMember.organization.name}`);
        } else {
          console.log(`⚠️  No organization found for landlord ${property.landlordId}, property ${property.id} skipped`);
        }
      }
    }
    ...
    for (const request of rentalRequests) {
      if (request.tenantId) {
        ...
        if (groupMember) {
          ...
          console.log(`✅ Updated rental request ${request.id} to reference tenant group ${groupMember.tenantGroup.name}`);
        } else {
          console.log(`⚠️  No tenant group found for tenant ${request.tenantId}, rental request ${request.id} skipped`);
        }
      }
    }
    ...
    for (const offer of offers) {
      ...
      if (offer.landlordId) {
        ...
        if (orgMember) {
          ...
          console.log(`✅ Offer ${offer.id}: landlord ${offer.landlordId} → organization ${orgMember.organization.name}`);
        }
      }
      ...
      if (offer.tenantId) {
        ...
        if (groupMember) {
          ...
          console.log(`✅ Offer ${offer.id}: tenant ${offer.tenantId} → tenant group ${groupMember.tenantGroup.name}`);
        }
      }
    ...
    for (const lease of leases) {
      if (lease.tenantId) {
        ...
        if (groupMember) {
          ...
          console.log(`✅ Updated lease ${lease.id} to reference tenant group ${groupMember.tenantGroup.name}`);
        } else {
          console.log(`⚠️  No tenant group found for tenant ${lease.tenantId}, lease ${lease.id} skipped`);
        }
      }
    }
    ...
    for (const payment of payments) {
      if (payment.userId) {
        ...
        if (groupMember) {
          ...
          console.log(`✅ Updated payment ${payment.id} to reference tenant group ${groupMember.tenantGroup.name}`);
        } else {
          console.log(`⚠️  No tenant group found for user ${payment.userId}, payment ${payment.id} skipped`);
        }
      }
    }
    ...
    for (const rentPayment of rentPayments) {
      if (rentPayment.userId) {
        ...
        if (groupMember) {
          ...
          console.log(`✅ Updated rent payment ${rentPayment.id} to reference tenant group ${groupMember.tenantGroup.name}`);
        } else {
          console.log(`⚠️  No tenant group found for user ${rentPayment.userId}, rent payment ${rentPayment.id} skipped`);
        }
      }
    }
    ...
    for (const review of reviews) {
      if (review.targetUserId) {
        ...
        if (groupMember) {
          ...
          console.log(`✅ Updated review ${review.id} to reference tenant group ${groupMember.tenantGroup.name}`);
        } else {
          console.log(`⚠️  No tenant group found for target user ${review.targetUserId}, review ${review.id} skipped`);
        }
      }
    }
    ...
    for (const match of matches) {
      if (match.landlordId) {
        ...
        if (orgMember) {
          ...
          console.log(`✅ Updated match ${match.id} to reference organization ${orgMember.organization.name}`);
        } else {
          console.log(`⚠️  No organization found for landlord ${match.landlordId}, match ${match.id} skipped`);
        }
      }
    }
