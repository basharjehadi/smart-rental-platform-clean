...
async function testOrganizationAPI() {
  ...
  try {
    ...
    allProperties.forEach(prop => {
      ...
      console.log(`    Organization: ${prop.organization ? prop.organization.name : 'None'} (${prop.organizationId})`);
    });
    ...
    if (availableUsers.length > 0) {
      ...
      console.log(`✅ Created test property: ${testProperty.name} owned by ${testOrg.name}`);
    ...
    console.log('1. User calls POST /api/organizations/upgrade with company details');
    console.log('2. System creates new Organization record');
    console.log('3. User is added as OWNER in OrganizationMember table');
    console.log('4. All existing properties are migrated to the new organization');
