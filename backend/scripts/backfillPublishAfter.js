...
async function backfillPublishAfter(dryRun = false) {
  ...
  try {
    console.log(`🔄 Starting publishAfter backfill... ${dryRun ? '(DRY RUN)' : ''}`);
    ...
    console.log(`🔍 Found ${result.reviewsToUpdate} reviews needing publishAfter backfill`);
    ...
    for (const [leaseId, reviews] of reviewsByLease) {
      try {
        if (leaseId === 'no-lease') {
          console.log(`⚠️  Skipping reviews without lease (${reviews.length} reviews)`);
          continue;
        }
        ...
        console.log(`📅 Processing lease ${leaseId} with end date: ${lease.endDate.toISOString()}`);

        // Calculate publishAfter for this lease
        const publishAfter = addDays(lease.endDate, 14);
        console.log(`   📅 Calculated publishAfter: ${publishAfter.toISOString()}`);

        // Update all reviews for this lease
        for (const review of reviews) {
          try {
            if (dryRun) {
              console.log(`   🔍 [DRY RUN] Would update review ${review.id}: publishAfter = ${publishAfter.toISOString()}`);
            } else {
              await prisma.review.update({
                where: { id: review.id },
                data: { publishAfter }
              });
              console.log(`   ✅ Updated review ${review.id}: publishAfter = ${publishAfter.toISOString()}`);
    ...
    if (dryRun) {
      console.log(`   [DRY RUN] Would update: ${result.reviewsToUpdate} reviews`);
    } else {
