...
try {
  ...
  if (cancelled.count > 0) log(true, 'Cancelled open renewals', String(cancelled.count));

  // 3) Renewal: tenant requests 12 months
  const reqTermMonths = 12;
  const createRenewal = await tenantHttp.post(`/leases/${leaseId}/renewals`, { proposedTermMonths: reqTermMonths, note: 'auto-test' });
  ...
  log(!!latest && latest.status === 'PENDING', 'Latest renewal is PENDING', latest?.id);

  // 5) Landlord counters with rent
  const counterRent = 1234;
  const counterRes = await landlordHttp.post(`/renewals/${latest.id}/counter`, { proposedMonthlyRent: counterRent, proposedTermMonths: reqTermMonths, note: 'auto-counter' });
  log(counterRes.data?.success, 'Landlord countered renewal');

  // 6) Tenant accepts
  const acceptRes = await tenantHttp.post(`/renewals/${counterRes.data?.renewal?.id || latest.id}/accept`, {});
  ...
  const leases = await prisma.lease.findMany({ where: { offerId: offer.id }, orderBy: { createdAt: 'desc' } });
  log(leases.length >= 1, 'Lease records present for offer', String(leases.length));

  // 8) Termination: tenant sends 30-day notice
  const effTenant = new Date(); effTenant.setDate(effTenant.getDate() + 30);
  // Clean any existing termination state to ensure idempotent test
  await prisma.lease.updateMany({ where: { id: leaseId }, data: { terminationNoticeByUserId: null, terminationNoticeDate: null, terminationEffectiveDate: null, terminationReason: null } });
  const termTenant = await tenantHttp.post(`/leases/${leaseId}/termination/notice`, { reason: 'auto-tenant', effectiveDate: effTenant.toISOString() });
  ...
  const effLand = new Date(); effLand.setDate(effLand.getDate() + 30);
  let landOk = true;
  try {
    const termLand = await landlordHttp.post(`/leases/${leaseId}/termination/notice`, { reason: 'auto-landlord', effectiveDate: effLand.toISOString() });
  ...
  const tenantNotifs = await prisma.notification.findMany({ where: { userId: offer.tenantId, createdAt: { gte: since } } });
  const landNotifs = await prisma.notification.findMany({ where: { userId: offer.landlordId, createdAt: { gte: since } } });
  log(tenantNotifs.length > 0, 'Tenant notifications created', String(tenantNotifs.length));
  log(landNotifs.length > 0, 'Landlord notifications created', String(landNotifs.length));
