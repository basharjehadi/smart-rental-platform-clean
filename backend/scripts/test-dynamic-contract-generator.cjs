  ...
  try {
    ...
    offers.forEach((offer, index) => {
      ...
      if (offer.property?.organization) {
        console.log(`    🏢 Landlord: BUSINESS (${offer.property.organization.name})`);
        console.log(`      Tax ID: ${offer.property.organization.taxId || 'N/A'}`);
        console.log(`      Reg Number: ${offer.property.organization.regNumber || 'N/A'}`);
        console.log(`      Has Signature: ${!!offer.property.organization.signatureBase64}`);
      } else {
      ...
      if (offer.organization) {
        ...
        console.log(`      Reg Number: ${offer.organization.regNumber || 'N/A'}`);
        console.log(`      Has Signature: ${!!offer.organization.signatureBase64}`);
      } else if (offer.tenantGroup?.members && offer.tenantGroup.members.length > 0) {
        ...
        offer.tenantGroup.members.forEach(member => {
          ...
          console.log(`          Has Signature: ${!!member.user.signatureBase64}`);
        });
      } else {
    ...
    if (offers.length > 0) {
      ...
      const isTenantGroup = !!testOffer.tenantGroup && testOffer.tenantGroup.members && testOffer.tenantGroup.members.length > 0;
      ...
      if (isTenantBusiness) {
        ...
        tenantData = {
          ...
          primarySignature: group.members.find(m => m.isPrimary)?.user.signatureBase64 ? true : false
        };
      ...
      if (isTenantBusiness && testOffer.rentalRequest?.occupants) {
        ...
        console.log('✅ Business tenant occupants data prepared:', occupantsData);
      } else if (isTenantGroup) {
    ...
    console.log('✅ Scenario detection (business/individual landlord, business/group/individual tenant)');
    console.log('✅ Dynamic data preparation based on contract type');
    console.log('✅ Business entity support (company name, tax ID, registration number)');
...
testDynamicContractGenerator()
  .then(() => {
    console.log('✅ Dynamic Contract Generator test script completed successfully');
    process.exit(0);
  })
  .catch((error) => {
