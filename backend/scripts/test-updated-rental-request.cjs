  ...
  try {
    ...
    organizations.forEach(org => {
      ...
      org.members.forEach(member => {
        console.log(`      * ${member.user.name || member.user.id} (${member.role})`);
      });
    });
    ...
    rentalRequests.forEach(request => {
      ...
      console.log(`    Tenant Group: ${request.tenantGroup.name} (${request.tenantGroupId})`);
    ...
    if (availableUsers.length > 0) {
      ...
      console.log('✅ This user would automatically get a tenant group when creating a rental request');
      console.log('✅ The rental request would be linked to the new tenant group');
    } else {
      console.log('ℹ️  All users are already in organizations or tenant groups');
    }
    ...
    if (businessUsers.length > 0) {
      ...
      console.log('✅ When creating rental request, they would need to provide occupants array');
    ...
    console.log('✅ Private tenants automatically get tenant groups when creating rental requests');
    console.log('✅ Business tenants must provide occupants array and link to organization');
...
testUpdatedRentalRequest()
  .then(() => {
    console.log('✅ Updated Rental Request Logic test script completed successfully');
    process.exit(0);
  })
  .catch((error) => {
