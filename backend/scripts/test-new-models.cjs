...
async function testNewModels() {
  ...
  try {
    ...
    if (organizations.length > 0) {
      ...
      console.log(`✅ Created test property: ${testProperty.name} owned by ${organizations[0].name}`);
    ...
    if (tenantGroups.length > 0) {
      ...
      if (testUser) {
        ...
        console.log(`✅ Added ${testUser.name || testUser.id} to tenant group ${tenantGroups[0].name}`);
        ...
        console.log(`ℹ️  No available users to test tenant group member creation`);
      }
    }
