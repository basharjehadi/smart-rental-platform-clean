  ...
  try {
    ...
    try {
      const availabilitySummary =
        await propertyAvailabilityService.getAvailabilitySummary();
      ...
      status.checks.propertyAvailability = {
        status: 'UNHEALTHY',
        message: 'Property availability service error',
        error: error.message,
      };
      status.status = 'DEGRADED';
    }
    ...
    try {
      ...
      status.checks.activeRequests = {
        status: 'UNHEALTHY',
        message: 'Active requests count error',
        error: error.message,
      };
      status.status = 'DEGRADED';
    }
    ...
    try {
      ...
      status.checks.availableProperties = {
        status: 'UNHEALTHY',
        message: 'Available properties count error',
        error: error.message,
      };
      status.status = 'DEGRADED';
    }
    ...
    try {
      ...
      status.checks.pendingOffers = {
        status: 'UNHEALTHY',
        message: 'Pending offers count error',
        error: error.message,
      };
      status.status = 'DEGRADED';
    }
    ...
    try {
      ...
      status.checks.activeMatches = {
        status: 'UNHEALTHY',
        message: 'Active matches count error',
        error: error.message,
      };
      status.status = 'DEGRADED';
    }
    ...
    try {
      ...
      status.checks.performance = {
        status: 'UNHEALTHY',
        message: 'Performance metrics error',
        error: error.message,
      };
      status.status = 'DEGRADED';
    }
...
const getPerformanceMetrics = async () => {
  try {
    ...
    const expiredRequests = await prisma.rentalRequest.count({
      where: { poolStatus: 'EXPIRED' },
    });
    const matchedRequests = await prisma.rentalRequest.count({
      where: { poolStatus: 'MATCHED' },
    });
    ...
    metrics.requests = {
      ...
      successRate:
        totalRequests > 0
          ? ((matchedRequests / totalRequests) * 100).toFixed(2)
          : 0,
    };
    ...
    metrics.offers = {
      ...
      conversionRate:
        totalOffers > 0 ? ((paidOffers / totalOffers) * 100).toFixed(2) : 0,
    };
    ...
    const respondedMatches = await prisma.landlordRequestMatch.count({
    ...
    metrics.matching = {
      ...
      responseRate:
        totalMatches > 0
          ? ((respondedMatches / totalMatches) * 100).toFixed(2)
          : 0,
    };
...
export const testSystemFunctionality = async (req, res) => {
  try {
    ...
    try {
      ...
      tests.requestPool = {
        status: 'PASSED',
        message: 'Request pool service working',
        stats: poolStats,
      };
    } catch (error) {
      tests.requestPool = {
        status: 'FAILED',
        message: 'Request pool service failed',
        error: error.message,
      };
    }
    ...
    try {
      const availabilitySummary =
        await propertyAvailabilityService.getAvailabilitySummary();
      tests.propertyAvailability = {
        status: 'PASSED',
        message: 'Property availability service working',
        summary: availabilitySummary,
      };
    } catch (error) {
      tests.propertyAvailability = {
        status: 'FAILED',
        message: 'Property availability service failed',
        error: error.message,
      };
    }
