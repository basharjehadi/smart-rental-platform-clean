...
async function debugTenants() {
  try {
    ...
    allOffers.forEach((offer) => {
      console.log(
        `Offer ID: ${offer.id}, Status: ${offer.status}, Landlord: ${offer.landlordId}, Tenant: ${offer.rentalRequest?.tenant?.name}`
      );
    });
    ...
    paidOffers.forEach((offer) => {
      console.log(
        `Paid Offer ID: ${offer.id}, Landlord: ${offer.landlordId}, Tenant: ${offer.rentalRequest?.tenant?.name}`
      );
    });
    ...
    users.forEach((user) => {
      console.log(
        `Landlord ID: ${user.id}, Name: ${user.name}, Email: ${user.email}`
      );
    });
  } catch (error) {
