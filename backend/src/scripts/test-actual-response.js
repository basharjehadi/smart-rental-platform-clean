...
async function testActualAPIResponse() {
  try {
    ...
    const dashboardResponse = await axios.get(
      'http://localhost:3001/api/tenant-dashboard/dashboard',
      {
        headers: {
    ...
    const offerResponse = await axios.get(
      `http://localhost:3001/api/tenant/offer/${offerId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    console.log(
      ...
      !!offer.landlord?.signatureBase64
    );
    console.log(
      'Tenant signature length:',
      offer.tenant?.signatureBase64?.length || 0
    );
    console.log(
      'Landlord signature length:',
      offer.landlord?.signatureBase64?.length || 0
    );

    if (offer.tenant?.signatureBase64) {
      console.log(
        'Tenant signature starts with:',
        offer.tenant.signatureBase64.substring(0, 50) + '...'
      );
    }

    if (offer.landlord?.signatureBase64) {
      console.log(
        'Landlord signature starts with:',
        offer.landlord.signatureBase64.substring(0, 50) + '...'
      );
    }
