...
import { moderateReviewText } from '../services/moderation.js';
...
async function testModerationIntegration() {
    ...
    try {
      ...
      switch (scenario.action) {
        case 'create':
          ...
          break;
        case 'submit':
          result = await controller.submitReview(
            scenario.reviewId,
            scenario.text
          );
          break;
        case 'edit':
          result = await controller.editReviewText(
            scenario.reviewId,
            scenario.text
          );
          break;
      ...
      if (scenario.expectedReasons) {
        ...
        reasonsMatch =
          JSON.stringify(actualReasons.sort()) ===
          JSON.stringify(scenario.expectedReasons.sort());
      }
      ...
      if (successMatches && reasonsMatch) {
        ...
        console.log('❌ FAILED');
        console.log(
          `  Expected: success=${scenario.expectedSuccess}, reasons=${JSON.stringify(scenario.expectedReasons || [])}`
        );
        console.log(
          `  Got: success=${result.success}, reasons=${JSON.stringify(result.reasons || [])}`
        );
      }
      ...
      if (result.success) {
        ...
        if (result.data) {
          console.log(
            `  Review ID: ${result.data.id}, Status: ${result.data.status}`
          );
        }
      } else {
  ...
  allReviews.forEach((review) => {
    console.log(
      `  Review ${review.id}: Status=${review.status}, ViolatesPolicy=${review.violatesPolicy}`
    );
  ...
  if (passedTests === totalTests) {
    console.log(
      '🎉 All tests passed! Moderation integration is working correctly.'
    );
  } else {
