...
async function testTenantGroupAPI() {
  ...
  try {
    ...
    members.forEach((member) => {
      console.log(
        `  - ${member.user.name || member.user.id} (${member.isPrimary ? 'PRIMARY' : 'MEMBER'}) in ${member.tenantGroup.name}`
      );
    });
    ...
    if (availableUsers.length > 0) {
      ...
      console.log(
        `✅ Created test tenant group: ${testTenantGroup.name} (${testTenantGroup.id})`
      );
      ...
      if (availableUsers.length > 1) {
        ...
        console.log(
          `✅ Invitation prepared for ${inviteUser.name || inviteUser.id}`
        );
        ...
        console.log(
          `✅ Transferred primary membership to ${inviteUser.name || inviteUser.id}`
        );
        ...
        console.log(
          `✅ Transferred primary membership back to ${testUser.name || testUser.id}`
        );
        ...
        console.log(
          `✅ Removed ${inviteUser.name || inviteUser.id} from the group`
        );
      }
