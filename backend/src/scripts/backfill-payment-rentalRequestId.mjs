...
async function backfillRentalRequestOnPayments() {
  ...
  try {
    ...
    for (const p of payments) {
      try {
        ...
        if (offer?.rentalRequestId) {
          ...
          updated += 1;
          console.log(
            `✅ Updated payment ${p.id} (${p.purpose}) → rentalRequestId ${offer.rentalRequestId}`
          );
        } else {
          console.log(
            `⚠️ Offer not found or missing rentalRequestId for payment ${p.id}`
          );
        }
      } catch (e) {
    ...
    console.log(
      `🏁 Backfill complete. Updated ${updated}/${payments.length} payments.`
    );
  } catch (error) {
