  ...
  try {
    ...
    if (organizations.length === 0) {
      ...
      if (sampleUser) {
        ...
        console.log(
          `✅ Added ${sampleUser.name || sampleUser.id} as organization owner`
        );
      }
    }
    ...
    if (tenantGroups.length === 0) {
      ...
      if (sampleTenant) {
        ...
        console.log(
          `✅ Added ${sampleTenant.name || sampleTenant.id} as primary tenant group member`
        );
      }
    }
    ...
    console.log(
      '3. You can now use the new Organization and TenantGroup models'
    );
