...
async function checkMatching() {
  try {
    ...
    rentalRequests.forEach((request) => {
      ...
      console.log(
        `   💰 Budget Range: ${request.budgetFrom} - ${request.budgetTo} PLN`
      );
      ...
      properties.forEach((property) => {
        ...
        const budgetMatch =
          property.monthlyRent >=
            (request.budgetFrom || request.budget * 0.8) &&
        ...
        // Date matching
        const dateMatch =
          new Date(property.availableFrom) <= new Date(request.moveInDate);
        ...
        console.log(
          `      📍 Location: ${locationMatch ? '✅' : '❌'} (Property: "${property.city}", Request: "${request.location}")`
        );
        console.log(
          `      💰 Budget: ${budgetMatch ? '✅' : '❌'} (Property: ${property.monthlyRent}, Request: ${request.budgetFrom}-${request.budgetTo})`
        );
        console.log(
          `      🏠 Type: ${typeMatch ? '✅' : '❌'} (Property: "${property.propertyType}", Request: "${request.propertyType}")`
        );
        console.log(
          `      📅 Date: ${dateMatch ? '✅' : '❌'} (Property: ${property.availableFrom}, Request: ${request.moveInDate})`
        );
        console.log(
          `      🛏️ Bedrooms: ${bedroomsMatch ? '✅' : '❌'} (Property: ${property.bedrooms}, Request: ${request.bedrooms})`
        );
        console.log(
          `      🎯 Overall: ${allMatch ? '✅ MATCH!' : '❌ No Match'}`
        );
      });
    });
  } catch (error) {
