...
async function testReviewPublisher() {
  ...
  try {
    ...
    console.log(
      `   Review 1 status: ${updatedReview1.status} (expected: PUBLISHED)`
    );
    console.log(
      `   Review 2 status: ${updatedReview2.status} (expected: PUBLISHED)`
    );

    if (
      updatedReview1.status === 'PUBLISHED' &&
      updatedReview2.status === 'PUBLISHED'
    ) {
