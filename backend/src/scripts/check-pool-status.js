...
async function checkPoolStatus() {
  try {
    ...
    matches.forEach((match) => {
      console.log(
        `  - Landlord: ${match.landlord.name} (${match.landlord.email})`
      );
    ...
    if (
      ...
      console.log(
        '  ❌ Rental request is in pool but has no matches - this indicates a bug!'
      );

      // Let's manually trigger the pool service to see what happens
      console.log('\n🔧 Attempting to manually add to pool...');
      try {
        const { default: requestPoolService } = await import(
