...
async function main() {
  try {
    ...
    const cityToken =
      rawLocation.split(',').pop()?.trim() || rawLocation.trim();
    ...
    const availableCutoff = moveIn
      ? new Date(moveIn.getTime() + 30 * 86400000)
      : null;

    const totalProps = await prisma.property.count();
    const statusAvail = await prisma.property.count({
    ...
    const budgetFiltered = await prisma.property.count({
      where: {
        ...
        monthlyRent: {
          lte:
            maxBudget != null
              ? Math.round(parseFloat(maxBudget) * 1.2)
              : 9999999,
          ...(minBudget != null ? { gte: parseFloat(minBudget) } : {}),
        },
      },
    });
    ...
    console.log(
      '- Combined (current logic + null-friendly date):',
      fullWhereCount
    );
  } catch (e) {
