...
async function testEnhancedContractTemplate() {
  ...
  try {
    ...
    const templatePath = path.join(
      __dirname,
      '../src/templates/contractTemplate.html'
    );
    ...
    if (
      templateContent.includes('{{#if isTenantBusiness}}') &&
    ...
    if (
      templateContent.includes('{{#if isTenantGroup}}') &&
      templateContent.includes('{{/if}}')
    ) {
    ...
    if (
      templateContent.includes('{{#if isLandlordBusiness}}') &&
      templateContent.includes('{{/if}}')
    ) {
      console.log(
        '✅ Landlord business conditional blocks properly structured'
      );
    } else {
      console.log(
        '❌ Landlord business conditional blocks missing or malformed'
      );
    }
    ...
    if (
      templateContent.includes('{{#each occupants}}') &&
    ...
    if (
      templateContent.includes('{{#each tenantGroupMembers}}') &&
      templateContent.includes('{{/each}}')
    ) {
    ...
    console.log(
      '✅ Conditional rendering for business/individual landlords and tenants'
    );
...
testEnhancedContractTemplate()
  .then(() => {
    console.log(
      '✅ Enhanced Contract Template test script completed successfully'
    );
    process.exit(0);
  })
  .catch((error) => {
