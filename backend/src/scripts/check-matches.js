...
async function checkAndFixMatches() {
  try {
    ...
    existingMatches.forEach((match) => {
      console.log(
        `   👤 Landlord: ${match.landlord.email} -> Request: ${match.rentalRequest.title}`
      );
    });
    ...
    const locationMatch = landlordProperty.city
      .toLowerCase()
      .includes(
        rentalRequest.location.split(',')[1]?.trim().toLowerCase() ||
          rentalRequest.location.toLowerCase()
      );
    const budgetMatch =
      landlordProperty.monthlyRent >=
        (rentalRequest.budgetFrom || rentalRequest.budget * 0.8) &&
      landlordProperty.monthlyRent <=
        (rentalRequest.budgetTo || rentalRequest.budget * 1.2);
    const typeMatch =
      !rentalRequest.propertyType ||
      landlordProperty.propertyType.toLowerCase() ===
        rentalRequest.propertyType.toLowerCase();
    const dateMatch =
      new Date(landlordProperty.availableFrom) <=
      new Date(rentalRequest.moveInDate);
    const bedroomsMatch =
      !rentalRequest.bedrooms ||
      landlordProperty.bedrooms === rentalRequest.bedrooms;

    console.log(`\n🔍 Match Analysis:`);
    console.log(
      `   📍 Location: ${locationMatch ? '✅' : '❌'} (Property: "${landlordProperty.city}", Request: "${rentalRequest.location}")`
    );
    console.log(
      `   💰 Budget: ${budgetMatch ? '✅' : '❌'} (Property: ${landlordProperty.monthlyRent}, Request: ${rentalRequest.budgetFrom}-${rentalRequest.budgetTo})`
    );
    console.log(
      `   🏠 Type: ${typeMatch ? '✅' : '❌'} (Property: "${landlordProperty.propertyType}", Request: "${rentalRequest.propertyType}")`
    );
    console.log(
      `   📅 Date: ${dateMatch ? '✅' : '❌'} (Property: ${landlordProperty.availableFrom}, Request: ${rentalRequest.moveInDate})`
    );
    console.log(
      `   🛏️ Bedrooms: ${bedroomsMatch ? '✅' : '❌'} (Property: ${landlordProperty.bedrooms}, Request: ${rentalRequest.bedrooms})`
    );

    const shouldMatch =
      locationMatch && budgetMatch && typeMatch && dateMatch && bedroomsMatch;
