...
async function debugLocation() {
  try {
    ...
    const test1 = rentalRequest.location
      .toLowerCase()
      .includes(property.city.toLowerCase());
    console.log(
      `1. Direct contains: ${test1} ("${rentalRequest.location}" contains "${property.city}")`
    );

    // Test 2: Prisma contains with exact case
    const test2 = rentalRequest.location.includes(property.city);
    console.log(
      `2. Prisma contains (exact): ${test2} ("${rentalRequest.location}" contains "${property.city}")`
    );

    // Test 3: Prisma contains with lowercase
    const test3 = rentalRequest.location
      .toLowerCase()
      .includes(property.city.toLowerCase());
    console.log(
      `3. Prisma contains (lowercase): ${test3} ("${rentalRequest.location.toLowerCase()}" contains "${property.city.toLowerCase()}")`
    );

    // Test 4: Split and check parts
    const parts = rentalRequest.location.split(',');
    console.log(
      `4. Location parts: [${parts.map((p) => `"${p.trim()}"`).join(', ')}]`
    );
    ...
    const districtMatch =
      districtPart.toLowerCase() === property.city.toLowerCase();
    const cityContains = cityPart
      .toLowerCase()
      .includes(property.city.toLowerCase());
    const districtContains = districtPart
      .toLowerCase()
      .includes(property.city.toLowerCase());
    ...
    console.log(
      `5. Prisma query with contains("${property.city}"): ${testQuery.length} results`
    );
    ...
    console.log(
      `6. Prisma query with contains("${property.city.toLowerCase()}"): ${testQueryLower.length} results`
    );
    ...
    console.log(
      `7. Prisma query with contains("${property.city.toUpperCase()}"): ${testQueryUpper.length} results`
    );
