...
function testMoveInReviewExclusion() {
  ...
  const endOfLeaseReview = testReviews.find(
    (r) => r.reviewStage === 'END_OF_LEASE'
  );

  if (moveInReview) {
    const transformed = transformReview(moveInReview);
    const hasNoScoreLabel = transformed.labels.some(
      (label) =>
  ...
  if (endOfLeaseReview) {
    ...
    const hasScoreLabel = transformed.labels.some(
      (label) =>
    ...
    if (hasScoreLabel) {
      console.log(
        '✅ END_OF_LEASE review correctly labeled as "affects score"'
      );
    } else {
