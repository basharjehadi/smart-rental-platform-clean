...
describe('Review System Integration Tests (Simplified)', () => {
  ...
  describe('1. Both sides submit → instant publish', () => {
    test('should publish reviews immediately when both sides submit', async () => {
      ...
      aggregatesService.computeUserAggregate.mockResolvedValue({
        success: true,
      });
  ...
  describe('3. MOVE_IN visible but not counted in aggregates', () => {
    test('should make MOVE_IN reviews visible but exclude from aggregates', async () => {
      ...
      await aggregatesService.computeUserAggregate(
        'landlord-1',
        'tenant-group-1'
      );

      // Verify that computeUserAggregate was called
      expect(aggregatesService.computeUserAggregate).toHaveBeenCalledWith(
        'landlord-1',
        'tenant-group-1'
      );
  ...
  describe('5. Content moderation blocks PII', () => {
    test('should block reviews containing PII', async () => {
      ...
      moderationService.moderateReviewText.mockReturnValue({
        ok: false,
        redactedText:
          'My phone number is [PHONE_REMOVED] and email is [EMAIL_REMOVED]. I live at [ADDRESS_REMOVED].',
        reasons: [
      ...
      const moderationResult = moderationService.moderateReviewText(
        reviewWithPII.text
      );

      expect(moderationResult.ok).toBe(false);
      expect(moderationResult.reasons).toContain(
        'Phone numbers are not allowed'
      );
  ...
  describe('6. Trust levels resolve correctly', () => {
    test('should calculate correct trust levels based on review data', async () => {
      ...
      const trustLevel =
        await trustLevelService.calculateTrustLevel('landlord-1');
    ...
    test('should handle new users with no reviews', async () => {
      ...
      const trustLevel =
        await trustLevelService.calculateTrustLevel('new-user-1');
    ...
    test('should handle users with many positive reviews', async () => {
      ...
      // Calculate trust level
      const trustLevel =
        await trustLevelService.calculateTrustLevel('experienced-user-1');
      ...
      const userBadges = [
        {
          id: 'badge-1',
          badge: {
            name: 'TENANT_ON_TIME_12M',
            description: 'On-time payments for 12 months',
          },
        },
        {
          id: 'badge-2',
          badge: {
            name: 'HOST_ACCURATE_95',
            description: '95% accuracy in property descriptions',
          },
        },
      ];
  ...
  describe('8. End-to-end review flow', () => {
    test('should handle complete review lifecycle', async () => {
      ...
      const publishedReview = await mockPrisma.review.findUnique({
        where: { id: review.id },
      });
