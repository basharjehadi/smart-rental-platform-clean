  ...
  refund: async (payment, prisma) => {
    ...
    try {
      ...
      await prisma.payment.update({
        where: { id: payment.id },
        data: { status: 'CANCELLED', errorMessage: `REFUNDED:${refund.id}` },
      });
...
const simpleMarkCancelled = async (payment, prisma, providerName) => {
  try {
    await prisma.payment.update({
      where: { id: payment.id },
      data: { status: 'CANCELLED', errorMessage: 'REFUNDED:' + providerName },
    });
...
const payuProvider = {
  refund: (payment, prisma) => simpleMarkCancelled(payment, prisma, 'PAYU'),
};
const p24Provider = {
  refund: (payment, prisma) => simpleMarkCancelled(payment, prisma, 'P24'),
};
const tpayProvider = {
  refund: (payment, prisma) => simpleMarkCancelled(payment, prisma, 'TPAY'),
};
...
export const getRefundProvider = (gateway) => {
  switch (gateway) {
    ...
    default:
      return {
        refund: async (payment, prisma) =>
          simpleMarkCancelled(payment, prisma, gateway || 'UNKNOWN'),
      };
  }
};
