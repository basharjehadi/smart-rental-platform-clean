...
class PropertyAvailabilityService {
  ...
  async updatePropertyAvailability(propertyId, availability, status = null) {
    try {
      ...
      if (property.organizationId) {
        ...
        await Promise.all(
          members.map((m) => this.updateUserAvailability(m.userId))
        );
      }

      console.log(
        `✅ Property ${propertyId} availability updated to ${availability}`
      );
      return property;
    } catch (error) {
  ...
  async updateUserAvailability(landlordId) {
    try {
      ...
      console.log(
        `📊 Landlord ${landlordId} availability updated to ${availableProperties > 0} (${availableProperties} available properties)`
      );
    } catch (error) {
  ...
  async bulkUpdatePropertyAvailability(landlordId, availability) {
    try {
      ...
      console.log(
        `✅ Updated ${result.count} properties for landlord ${landlordId} to availability: ${availability}`
      );
      return result;
    } catch (error) {
  ...
  async getAvailabilitySummary() {
    try {
      ...
      summary.landlords = {
        ...
        availabilityRate:
          totalLandlords > 0
            ? ((availableLandlords / totalLandlords) * 100).toFixed(2)
            : 0,
      };
