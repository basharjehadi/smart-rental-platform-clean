...
class LeaseStatusCheckerJob {
  ...
  async checkUrgentLeases() {
    try {
      ...
      const { getCurrentWarsawTime, addDaysUTC } = await import('../utils/dateUtils.js');
      ...
      if (endingSoon.length > 0) {
        console.log(`⚠️  Found ${endingSoon.length} leases ending within 24 hours`);
      ...
      if (terminated24H.length > 0) {
        console.log(`⏰ Found ${terminated24H.length} TERMINATED_24H leases now effective`);
  ...
  async handleLeaseEndingSoon(lease) {
    try {
      console.log(`⚠️  Lease ending soon: ${lease.id} (ends: ${lease.endDate})`);
      ...
      if (existingReviews.length === 0) {
        ...
        console.log(`ℹ️  End-of-lease reviews already exist for lease: ${lease.id}`);
      }
  ...
  async handleTerminationEffective(lease) {
    try {
      ...
      await leaseEventService.handleLeaseStatusChange(lease.id, 'TERMINATED_24H', 'TERMINATED', {
        reason: lease.terminationReason || '24-hour termination now effective',
      ...
      console.error(`❌ Error handling termination effective for lease ${lease.id}:`, error);
    }
  }
