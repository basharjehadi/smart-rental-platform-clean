  ...
  try {
    ...
    const review1 = await prisma.review.create({
      data: {
        ...
        status: 'PUBLISHED',
        publishedAt: new Date(now.getFullYear(), now.getMonth() - 6, now.getDate()),
        isDoubleBlind: true
      }
    });
    ...
    const review2 = await prisma.review.create({
      data: {
        ...
        targetTenantGroupId: testData.tenantGroupId,
        status: 'PUBLISHED',
        publishedAt: new Date(now.getFullYear() - 1, now.getMonth() - 6, now.getDate()),
        isDoubleBlind: true
      }
    });
    ...
    const review3 = await prisma.review.create({
      data: {
        ...
        targetTenantGroupId: testData.tenantGroupId,
        status: 'PUBLISHED',
        publishedAt: new Date(now.getFullYear() - 2, now.getMonth() - 6, now.getDate()),
        isDoubleBlind: true
      }
    });
    ...
    reviews.forEach((review, index) => {
      console.log(`   Review ${index + 1}: ${review.rating} stars, published ${review.publishedAt.toDateString()}`);
    });
...
async function testComputeUserAggregate() {
  ...
  try {
    ...
    if (result.averageRating !== null && result.totalReviews >= 3) {
      ...
      console.log('ℹ️  User aggregate computed but average is null (less than 3 reviews)');
    }
...
async function testGetUserAggregateSummary() {
  ...
  try {
    ...
    if (reviewBreakdown.recent > 0 && reviewBreakdown.medium > 0 && reviewBreakdown.old > 0) {
      console.log('✅ All time periods have reviews - perfect for testing time-weighting!');
    }
...
async function testInsufficientReviews() {
  ...
  try {
      ...
      data: [
        {
          ...
          targetTenantGroupId: testData.tenantGroupId,
          status: 'PUBLISHED',
          publishedAt: new Date(now.getFullYear(), now.getMonth() - 2, now.getDate()),
          isDoubleBlind: true
        },
        {
          rating: 5,
          ...
          targetTenantGroupId: testData.tenantGroupId,
          status: 'PUBLISHED',
          publishedAt: new Date(now.getFullYear(), now.getMonth() - 1, now.getDate()),
          isDoubleBlind: true
        }
      ]
    });
    ...
    if (result.averageRating === null && result.totalReviews === 2) {
      console.log('✅ Correctly returned null average for user with less than 3 reviews');
    } else {
