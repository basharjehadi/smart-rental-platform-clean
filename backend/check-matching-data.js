...
async function checkMatchingData() {
  try {
    ...
    console.log(`   Would match with current logic: ${wouldMatch ? '✅ YES' : '❌ NO'}`);
    
    if (wouldMatch) {
      console.log(`   Reason: Property rent (${property.monthlyRent}) <= 2x max budget (${budgetTo * 2})`);
    }
    ...
    if (extractedCity && property.city.toLowerCase().includes(extractedCity.toLowerCase())) {
      score += 40;
      console.log(`   ✅ Location match: +40 points`);
    } else {
      console.log(`   ❌ Location mismatch: Property city "${property.city}" vs extracted "${extractedCity}"`);
    }
    ...
    if (property.monthlyRent >= (budgetFrom - budgetTolerance) && 
        property.monthlyRent <= (budgetTo + budgetTolerance)) {
      score += 30;
      console.log(`   ✅ Budget match with tolerance: +30 points`);
    } else if (property.monthlyRent >= budgetFrom && property.monthlyRent <= budgetTo) {
      score += 25;
      ...
      console.log(`   ❌ Budget mismatch: Property ${property.monthlyRent} PLN vs range ${budgetFrom}-${budgetTo} PLN`);
    }
    
    // Bedrooms match (20 points)
    if (rentalRequest.bedrooms && property.bedrooms === rentalRequest.bedrooms) {
      score += 20;
      console.log(`   ✅ Bedrooms exact match: +20 points`);
    } else if (rentalRequest.bedrooms && Math.abs(property.bedrooms - rentalRequest.bedrooms) <= 1) {
      score += 10;
      console.log(`   ✅ Bedrooms close match: +10 points`);
    } else if (rentalRequest.bedrooms) {
      console.log(`   ❌ Bedrooms mismatch: Property ${property.bedrooms} vs request ${rentalRequest.bedrooms}`);
    }
    
    // Property type match (15 points)
    if (rentalRequest.propertyType && property.propertyType && 
        property.propertyType.toLowerCase().includes(rentalRequest.propertyType.toLowerCase())) {
      score += 15;
      console.log(`   ✅ Property type match: +15 points`);
    } else if (rentalRequest.propertyType) {
      console.log(`   ❌ Property type mismatch: Property "${property.propertyType}" vs request "${rentalRequest.propertyType}"`);
    }
