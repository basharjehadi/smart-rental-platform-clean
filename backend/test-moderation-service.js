...
import { moderateReviewText, enqueueTrustAndSafetyReview } from './src/services/moderation.js';
...
const testCases = [
  ...
  // Hate speech (should be blocked)
  {
    ...
    expectedOk: false,
    expectedReasons: ['Profanity or inappropriate language detected', 'Hate speech or violent content detected']
  },
...
function testModerationService() {
  ...
  testCases.forEach((testCase, index) => {
    ...
    try {
      ...
      const reasonsMatch = JSON.stringify(result.reasons.sort()) === JSON.stringify(testCase.expectedReasons.sort());
      ...
      if (okMatches && reasonsMatch) {
        ...
        console.log('❌ FAILED');
        console.log(`  Expected: ok=${testCase.expectedOk}, reasons=${JSON.stringify(testCase.expectedReasons)}`);
        console.log(`  Got: ok=${result.ok}, reasons=${JSON.stringify(result.reasons)}`);
      }
  ...
  if (passedTests === totalTests) {
    console.log('🎉 All tests passed! Moderation service is working correctly.');
  } else {
...
async function testTrustAndSafetyEnqueue() {
  ...
  try {
    ...
    await enqueueTrustAndSafetyReview(testReviewId, originalText, redactedText, reasons);
