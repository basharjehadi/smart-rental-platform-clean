...
async function debugMatching() {
  try {
    ...
    // Location matching
    const requestCity = rentalRequest.city || rentalRequest.location.split(',')[1]?.trim() || rentalRequest.location.trim();
    console.log(`  📍 Request City (extracted): "${requestCity}"`);
    console.log(`  🏠 Property City: "${property.city}"`);
    console.log(`  📍 Location Match: ${requestCity.toLowerCase() === property.city.toLowerCase() ? '✅ YES' : '❌ NO'}`);

    // Budget matching
    const maxBudget = rentalRequest.budgetTo * 1.5 || rentalRequest.budget * 1.5;
    ...
    console.log(`  💰 Budget Match: ${property.monthlyRent <= maxBudget ? '✅ YES' : '❌ NO'}`);
    ...
    console.log(`  📅 Date Match: ${availableFrom && availableFrom <= moveInDate ? '✅ YES' : '❌ NO'}`);
    ...
    matchingLandlords.forEach(landlord => {
      ...
      landlord.properties.forEach(prop => {
        console.log(`      🏢 ${prop.name} - ${prop.city}, ${prop.monthlyRent} PLN`);
      });
    });
