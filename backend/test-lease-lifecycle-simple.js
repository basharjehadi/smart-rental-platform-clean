...
async function findActiveLease(tenantToken) {
  ...
  const dashboard = await getWithAuth('/tenant/dashboard', tenantToken);
...
async function testTenantResponse(tenantToken, renewalId, accept = true) {
  console.log(`\n👤 Testing Tenant ${accept ? 'Acceptance' : 'Decline'}...`);
  ...
  try {
    const response = await postWithAuth(
      ...
      { 
        accepted: accept, 
        note: `Test ${accept ? 'acceptance' : 'decline'}` 
      },
      tenantToken
    );
    
    console.log(`✅ Tenant ${accept ? 'accepted' : 'declined'} proposal`);
    return response;
    
  } catch (error) {
    console.error(`❌ Tenant ${accept ? 'acceptance' : 'decline'} failed:`, error.message);
    throw error;
  }
}
