...
async function testSimple() {
  try {
    ...
    // Step 3: Create rental request with move-in date further in the future
    console.log('\n3️⃣ Creating rental request...');
    const moveInDate = new Date();
    moveInDate.setDate(moveInDate.getDate() + 10); // 10 days from now to ensure expiration > now
    ...
    matches.forEach(match => {
      console.log(`   - Match: ${match.id}, Organization: ${match.organizationId}, Score: ${match.matchScore}`);
    });

    // Step 5: Test getRequestsForLandlordUser method directly
    console.log('\n5️⃣ Testing getRequestsForLandlordUser method...');
    
    // Check if the method exists
    if (typeof requestPoolService.getRequestsForLandlordUser === 'function') {
      console.log('✅ Method exists');
      const requests = await requestPoolService.getRequestsForLandlordUser(user.id, 1, 20);
      console.log(`✅ getRequestsForLandlordUser returned ${requests.requests.length} requests`);
      ...
      if (requests.requests.length > 0) {
        ...
        console.log(`✅ No sensitive data exposed: ${!hasPhone && !hasEmail && !hasDOB}`);
      }
    } else {
      console.log('❌ Method does not exist');
      console.log('Available methods:', Object.getOwnPropertyNames(Object.getPrototypeOf(requestPoolService)));
    }

    // Step 6: Test markAsViewedForOrg
    console.log('\n6️⃣ Testing markAsViewedForOrg...');
    
    if (typeof requestPoolService.markAsViewedForOrg === 'function') {
      console.log('✅ Method exists');
      
      // First call
      await requestPoolService.markAsViewedForOrg(organization.id, rentalRequest.id);
      ...
      console.log(`👁️ View count after first call: ${requestAfterFirst.viewCount || 0}`);
      ...
      console.log(`👁️ View count after second call: ${requestAfterSecond.viewCount || 0}`);
      
      // Verify view count only incremented once
      const viewCountIncrement = (requestAfterSecond.viewCount || 0) - (requestAfterFirst.viewCount || 0);
      console.log(`✅ View count increment: ${viewCountIncrement} (should be 0 for duplicate calls)`);
    } else {
      console.log('❌ Method does not exist');
    }
