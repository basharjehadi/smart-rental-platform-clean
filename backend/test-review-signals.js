...
async function testRetrieveReviewSignals() {
  ...
  try {
    ...
    const groupSignals = await getTenantGroupReviewSignals(testData.tenantGroupId);
    ...
    console.log(`   ✅ Found ${paymentSignals.signals.length} PAYMENT_CONFIRMED signals`);

    const depositSignals = await getReviewSignalsByType('DEPOSIT_RETURNED');
    console.log(`   ✅ Found ${depositSignals.signals.length} DEPOSIT_RETURNED signals`);
...
async function testCleanupFunction() {
  ...
  try {
    ...
    console.log(`   ✅ Cleanup function executed, deleted ${deletedCount} old signals`);
