...
const updatedText = 'Updated review: Excellent experience with this landlord! Very responsive, professional, and the property exceeded expectations.';
...
async function testValidationErrors(reviewId) {
  ...
  try {
    const response = await fetch(`${BASE_URL}/api/reviews/${reviewId}/submit`, {
      ...
      body: JSON.stringify({ stars: 6, text: 'Test comment with invalid stars' })
    });
...
async function runAPITests() {
  ...
  if (!TEST_TOKEN || TEST_TOKEN === 'your-test-jwt-token-here') {
    ...
    console.log('   You can get one by logging in through your frontend or API');
    return;
  }
