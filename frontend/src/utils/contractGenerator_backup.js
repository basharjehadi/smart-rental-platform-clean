...
export const generateRentalContract = (offer, user = null) => {
  ...
  console.log('Offer tenant signature:', (offer.tenant?.signatureBase64 || offer.rentalRequest?.tenant?.signatureBase64) ? 'Present' : 'Missing');
  console.log('Offer landlord signature:', offer.landlord?.signatureBase64 ? 'Present' : 'Missing');
  console.log('Tenant signature length:', (offer.tenant?.signatureBase64 || offer.rentalRequest?.tenant?.signatureBase64)?.length || 0);
  ...
  // Generate landlord-friendly payment schedule (same as tenant dashboard)
  const generatePaymentSchedule = () => {
    ...
    currentDate.setDate(1); // Set to 1st of the month for consistent calculation
    ...
    while (currentDate < endDate) {
      ...
      if (isLastMonth) {
        const daysInLastMonth = endDate.getDate(); // Days in last month
        amount = Math.round((monthlyRent * daysInLastMonth) / 30); // Prorate based on 30 days
  ...
  const contractData = {
    ...
    // Tenant information - use actual data from offer.rentalRequest.tenant (provided by backend)
    ...
    tenantPESEL: offer.rentalRequest?.tenant?.pesel || offer.tenant?.pesel || null,
    ...
    tenantPhone: offer.rentalRequest?.tenant?.phoneNumber || offer.tenant?.phoneNumber || '+48123456789',
    tenantAddress: offer.rentalRequest?.tenant?.street || offer.rentalRequest?.tenant?.address || offer.tenant?.address || offer.tenant?.street || 'Tenant Address',
    tenantCity: offer.rentalRequest?.tenant?.city || offer.tenant?.city || 'City',
    tenantZipCode: offer.rentalRequest?.tenant?.zipCode || offer.tenant?.zipCode || '00-000',
  ...
  console.log('Landlord & Tenant Data Debug:', {
    ...
    tenantName: offer.rentalRequest?.tenant?.name || offer.tenant?.name,
    tenantEmail: offer.rentalRequest?.tenant?.email || offer.tenant?.email,
    tenantAddress: offer.rentalRequest?.tenant?.street || offer.rentalRequest?.tenant?.address || offer.tenant?.address,
    tenantCity: offer.rentalRequest?.tenant?.city || offer.tenant?.city,
        ...
        <div class="section">
          ...
          <div class="signature-section">
            <div class="signature-box">
              ...
              ${landlordSignature ? `<img src="data:image/png;base64,${landlordSignature}" class="signature-image" alt="Landlord Signature">` : '<div class="signature-line"></div>'}
            ...
            <div class="signature-box">
              ...
              ${tenantSignature ? `<img src="data:image/png;base64,${tenantSignature}" class="signature-image" alt="Tenant Signature">` : '<div class="signature-line"></div>'}
              <div class="signature-date">Date: ${contractData.date}</div>
            </div>
          </div>
        </div>
