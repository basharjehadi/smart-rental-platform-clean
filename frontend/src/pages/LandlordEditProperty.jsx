...
const LandlordEditProperty = () => {
  ...
  const propertyTypes = ['apartment', 'house', 'studio', 'room', 'shared room'];
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (['RENTED'].includes(property.status)) {
          setErrors({
            fetch:
              'This property is currently rented and cannot be edited. Please contact support if you need to make changes.',
          });
          setFetching(false);
          return;
        }
          ...
          if (property.houseRules) {
            existingAmenities =
              typeof property.houseRules === 'string'
                ...
                : property.houseRules;
            console.log(
              '🏠 Parsed amenities from houseRules:',
              existingAmenities
            );
          }
        } catch (error) {
        ...
        const formDataToSet = {
          ...
          amenities: Array.isArray(existingAmenities)
            ? existingAmenities.filter(Boolean)
            : [],
          street: street,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (
        ...
        originalMedia.imageUrls.length > 0
      ) {
      ...
      if (retainedImageUrls.length > 0) {
        submitData.append(
          'retainedImageUrls',
          JSON.stringify(retainedImageUrls)
        );
      }
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-gray-600 mt-1'>
                Update your property listing to attract more tenants
              </p>
            </div>
            ...
            <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6 relative'>
              ...
              <div className='mb-6 pr-32'>
                ...
                <div className='flex justify-between mt-4'>
                  <div
                    className={`flex flex-col items-center ${currentStep >= 1 ? 'text-black' : 'text-gray-400'}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center mb-1 ${currentStep >= 1 ? 'bg-black text-white' : 'bg-gray-200'}`}
                    >
                      {currentStep > 1 ? (
                        <svg
                          className='w-4 h-4'
                  ...
                  <div
                    className={`flex flex-col items-center ${currentStep >= 2 ? 'text-black' : 'text-gray-400'}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center mb-1 ${currentStep >= 2 ? 'bg-black text-white' : 'bg-gray-200'}`}
                    >
                      {currentStep > 2 ? (
                        <svg
                          className='w-4 h-4'
                  ...
                  <div
                    className={`flex flex-col items-center ${currentStep >= 3 ? 'text-black' : 'text-gray-400'}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center mb-1 ${currentStep >= 3 ? 'bg-black text-white' : 'bg-gray-200'}`}
                    >
                      <svg
                        className='w-4 h-4'
                        fill='none'
