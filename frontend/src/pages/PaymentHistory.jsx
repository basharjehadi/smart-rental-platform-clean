...
const PaymentHistory = () => {
  ...
  const fetchPaymentData = async () => {
    try {
      ...
      const historyResponse = await api.get('/tenant-dashboard/payments/history');
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {upcomingPayments.length > 0 ? formatDate(upcomingPayments[0].dueDate) : 'N/A'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">Standard monthly payments due on the 10th of each month</p>
              </div>
              <div className="flex items-center space-x-3">
                {upcomingPayments.length > 0 && (
                  ...
                  >
                    {selectedPayments.length === upcomingPayments.length ? 'Deselect All' : 'Select All'}
                  </button>
                )}
                {selectedPayments.length > 0 && (
                  ...
                  >
                    Pay {selectedPayments.length} Selected ({formatCurrency(getTotalSelectedAmount())})
                  </button>
                )}
              </div>
            </div>
            ...
            {upcomingPayments.length > 0 ? (
              <div className="space-y-4">
                {upcomingPayments.map((payment, index) => {
                  const isSelected = selectedPayments.find(p => p.month === payment.month);
                  return (
                    <div key={index} className={`flex items-center justify-between p-4 rounded-lg border-2 transition-colors ${
                      isSelected ? 'border-blue-500 bg-blue-50' : 
                      isOverdue(payment.dueDate) ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-transparent'
                    }`}>
                      <div className="flex items-center">
                        <input
                          type="checkbox"
                        ...
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-4 ${
                          isOverdue(payment.dueDate) ? 'bg-red-100' : 'bg-blue-100'
                        }`}>
                          <svg className={`w-5 h-5 ${
                            isOverdue(payment.dueDate) ? 'text-red-600' : 'text-blue-600'
                        ...
                        <div>
                          ...
                          <p className={`text-xs ${
                            isOverdue(payment.dueDate) ? 'text-red-600' : 'text-gray-600'
                          }`}>
                      ...
                      <div className="flex items-center space-x-3">
                        <div className="text-right">
                          <p className="text-sm font-medium text-gray-900">{formatCurrency(payment.amount)}</p>
                          {getStatusBadge(isOverdue(payment.dueDate) ? 'Overdue' : payment.status)}
                        </div>
                        <button
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Pay in Advance</h3>
              <button
            ...
            <div className="mb-6">
              <p className="text-sm text-gray-600 mb-4">
                You're about to pay {selectedPayments.length} month(s) in advance:
              </p>
              ...
              <div className="border-t pt-3">
                <div className="flex justify-between font-semibold">
                  ...
                  <span className="text-blue-600">{formatCurrency(getTotalSelectedAmount())}</span>
                </div>
              </div>
            </div>
