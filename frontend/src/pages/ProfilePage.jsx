...
const ProfilePage = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      setProfileImageUrl(userData.profileImage ? `http://localhost:3001/uploads/profile_images/${userData.profileImage}` : '');
      setIdentityDocumentUrl(userData.identityDocument ? `http://localhost:3001/uploads/identity_documents/${userData.identityDocument}` : '');
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      if (response.data && response.data.profileImage) {
        setProfileImageUrl(`http://localhost:3001/uploads/profile_images/${response.data.profileImage}`);
        setProfileImage(null);
        console.log('✅ Profile image uploaded successfully:', response.data.profileImage);
      } else {
        throw new Error('Invalid response from server');
      }
    } catch (error) {
      console.error('Error uploading image:', error);
      const errorMessage = error.response?.data?.error || error.message || 'Failed to upload profile image';
  ...
  const handleIdentityDocumentUpload = async () => {
    ...
    try {
      ...
      setIdentityDocumentUrl(`http://localhost:3001/uploads/identity_documents/${response.data.data.filename}`);
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                {userRole === 'TENANT' && (
                  <div className="border-b border-gray-200 pb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        <label htmlFor="kartaPobytuNumber" className="block text-sm font-medium text-gray-700 mb-2">
                          Karta Pobytu Number
                        </label>
                        <input
                          type="text"
                          ...
                          onChange={handleInputChange}
                          className={`w-full px-4 py-3 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors ${
                            formData.kartaPobytuNumber && !validateKartaPobytu(formData.kartaPobytuNumber) 
                              ? 'border-red-300' 
                              : 'border-gray-300'
                          }`}
                ...
                {userRole === 'LANDLORD' && (
                  <div className="border-b border-gray-200 pb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label htmlFor="dowodOsobistyNumber" className="block text-sm font-medium text-gray-700 mb-2">
                        ...
                        <input
                          ...
                          onChange={handleInputChange}
                          className={`w-full px-4 py-3 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors ${
                            formData.dowodOsobistyNumber && !validateDowodOsobisty(formData.dowodOsobistyNumber) 
                              ? 'border-red-300' 
                              : 'border-gray-300'
                          }`}
                ...
                <KYCDocumentUpload
                  ...
                  onDocumentUpload={(filename) => {
                    setIdentityDocumentUrl(`http://localhost:3001/uploads/identity_documents/${filename}`);
                    setIdentityDocument(null);
                  }}
