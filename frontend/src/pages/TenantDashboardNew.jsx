...
const TenantDashboardNew = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      console.log(
        '✅ Frontend: User is authenticated, fetching dashboard data...'
      );
      fetchDashboardData();
      fetchInvitations();
    } else {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        console.log(
          'Profile image type:',
          typeof response.data.user?.profileImage
        );
        console.log(
          'Profile image length:',
          response.data.user?.profileImage?.length
        );
        console.log(
          'Full profile user object:',
  ...
  useEffect(() => {
    ...
    if (profileData) {
      ...
      console.log(
        '- Constructed URL:',
        getProfilePhotoUrl(profileData.profileImage)
      );
    }
  }, [profileData]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      console.log(
        '✅ Frontend: Dashboard response received:',
        dashboardResponse.data
      );
      console.log(
        '🔍 Frontend: Offer ID from backend:',
        dashboardResponse.data.offerId
      );
      console.log(
        '🔍 Frontend: Offer ID type:',
        typeof dashboardResponse.data.offerId
      );
      console.log(
        '🔍 Frontend: Offer ID length:',
        dashboardResponse.data.offerId?.length
      );
      ...
      if (
        dashboardResponse.data?.leases &&
        dashboardResponse.data.leases.length > 0
      ) {
      ...
      console.log(
        '🔍 Frontend: Dashboard data after set:',
        dashboardResponse.data
      );
      console.log(
        '🔍 Frontend: Offer ID in dashboard data:',
        dashboardResponse.data.offerId
      );
    } catch (error) {
  ...
  const currentLease = focusedOfferId
    ? leases.find(l => l.offerId === focusedOfferId)
    : null;
  ...
  const latestOpen =
    latestRenewal &&
    (latestRenewal.status === 'PENDING' || latestRenewal.status === 'COUNTERED')
      ? latestRenewal
      : null;
  ...
  const handleViewContract = async () => {
    try {
      ...
      if (!dashboardData.hasActiveLease || !currentOfferId) {
        console.log(
          '❌ Dashboard: Missing data - hasActiveLease:',
          dashboardData.hasActiveLease,
          'offerId:',
          currentOfferId
        );
      ...
      if (typeof currentOfferId !== 'string' || currentOfferId.length < 20) {
        ...
        alert(
          'Invalid offer ID format. Please refresh the page and try again.'
        );
        return;
      }
            ...
            {dashboardData.hasActiveLease ? (
                      ...
                      <p className='text-lg font-bold text-gray-900 break-words leading-tight'>
                        {formatPropertyType(
                          currentProperty?.propertyType ||
                            dashboardData.property?.propertyType,
                          currentProperty?.rooms ||
                            dashboardData.property?.rooms
                        )}
                      </p>
                    </div>
                  </div>
              ...
              <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center'>
                ...
                <p className='text-gray-600 mb-6'>
                  You don't have any active rental properties yet. To create a
                  rental request, please go to the My Requests page from the
                  sidebar.
                </p>
                  ...
                  <div className='space-y-3'>
                    <div>
                      <p className='text-sm font-medium text-gray-900'>
                        {dashboardData.landlord?.name}
                      </p>
                      <p className='text-sm text-gray-600'>
                        {dashboardData.landlord?.company}
                      </p>
                    </div>
                    <div className='space-y-2'>
                      <p className='text-sm text-gray-600'>
                        {currentLandlord?.email ||
                          dashboardData.landlord?.email}
                      </p>
                      <p className='text-sm text-gray-600'>
                        {currentLandlord?.phone ||
                          dashboardData.landlord?.phone}
                      </p>
                      <p className='text-sm text-gray-600'>
                        {currentLandlord?.address ||
                          dashboardData.landlord?.address}
                      </p>
                    </div>
                    <button
                      onClick={() =>
                        navigate(
                          `/messaging?conversationId=new&propertyId=${currentProperty?.id || dashboardData.property?.id}`
                        )
                      }
                  ...
                  <div className='space-y-3'>
                    <p className='text-sm text-gray-600'>
                      {currentProperty?.address ||
                        dashboardData.property?.address}
                    </p>
                    <div className='grid grid-cols-2 gap-2 text-sm'>
                      ...
                      <div>
                        ...
                        {currentProperty?.bathrooms ||
                          dashboardData.property?.bathrooms}
                      </div>
                      <div>
                        <span className='font-medium'>Area:</span>{' '}
                        {formatArea(
                          currentProperty?.area || dashboardData.property?.area
                        )}
                      </div>
                      <div>
                        <span className='font-medium'>Lease Term:</span>{' '}
                        {currentProperty?.leaseTerm ||
                          dashboardData.property?.leaseTerm}{' '}
                        months
                      </div>
                    </div>
                    <div className='mt-4'>
                      ...
                      <div className='flex flex-wrap gap-2'>
                        {(
                          ...
                          dashboardData.property?.amenities
                        )?.map((amenity, index) => (
                          <span
                            key={index}
                    ...
                    <div className='space-y-2 bg-indigo-50/40 border border-indigo-200 rounded-lg p-3'>
                      ...
                      {renewals.length > 0 && (
                        <div className='bg-white border border-indigo-200 rounded p-3'>
                          ...
                          <div className='space-y-2 max-h-48 overflow-auto pr-1'>
                            {renewals
                              ...
                              .map(r => (
                                <div
                                  key={r.id}
                                  className='text-xs text-gray-700 bg-white border border-gray-100 rounded p-2'
                                >
                                  <div className='flex items-center justify-between'>
                                    <span className='font-medium'>
                                      {r.status}
                                  ...
                                  {r.proposedMonthlyRent != null && (
                                    <div className='mt-1'>
                                      Proposed rent: {r.proposedMonthlyRent} zł
                                    </div>
                                  )}
                                  {r.proposedTermMonths && (
                                    <div>
                                      Term: {r.proposedTermMonths} months
                                    </div>
                                  )}
                                  {r.proposedStartDate && (
                                    <div>
                          ...
                          {latestOpen &&
                            latestOpen.initiatorUserId !== user?.id && (
                              <div className='mt-2 flex space-x-2'>
                                ...
                                <button
                                  onClick={async () => {
                                    try {
                                      await api.post(
                                        `/renewals/${latestOpen.id}/decline`,
                                        {}
                                      );
                                      if (leaseMeta?.id) {
                                        const threadResp = await api.get(
              ...
              <RenewalRequestModal
                ...
                onSubmit={async ({ note }) => {
                  try {
                    ...
                    const metaResp = await api.get(
                      `/leases/by-offer/${currentOfferForMeta}`
                    );
                    const leaseId = metaResp.data?.lease?.id;
                    if (!leaseId) {
