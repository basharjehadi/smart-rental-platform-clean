...
const TenantProfile = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const polishOption = countriesWithFlags.find(
        country => country.value === 'PL'
      );
      const otherCountries = countryData
        .filter(country => country.value !== 'PL')
    ...
    try {
      ...
      if (response.data.user.profession) {
        const professionOption = professionOptions.find(
          option => option.value === response.data.user.profession
        );
        if (professionOption) {
          ...
          setSelectedProfession(
            professionOptions.find(option => option.value === 'Other')
          );
      ...
      if (response.data.user.badges) {
        ...
        console.log('🔍 Tenant Profile Badges:', response.data.user.badges);
      } else {
        setBadges([]);
        console.log('🔍 No badges found in tenant profile');
      }
    } catch (error) {
  ...
  const handleKycSubmit = async () => {
    ...
    if (!gdprConsent) {
      setKycSubmissionError(
        'You must accept GDPR / RODO terms before submitting.'
      );
      return;
    }
    ...
    try {
      ...
      const backResponse = await api.post(
        '/users/upload-identity-document',
        backFormData,
        {
          headers: {
      ...
      setKycSubmissionSuccess(
        'KYC documents submitted successfully! Your identity verification is now being processed.'
      );
    } catch (error) {
      ...
      if (error.response?.status === 400) {
        setKycSubmissionError(
          `Upload failed: ${error.response.data?.message || 'Invalid file format or size'}`
        );
      } else if (error.response?.status === 401) {
        setKycSubmissionError('Authentication failed. Please log in again.');
      } else if (error.response?.status === 413) {
        setKycSubmissionError(
          'File too large. Please use files smaller than 10MB.'
        );
  ...
  const handlePESELChange = value => {
    ...
    if (
      selectedCitizenship?.value === 'Polish' ||
      selectedCitizenship?.label?.includes('Polish')
    ) {
  ...
  const validateField = (name, value, citizenship) => {
    ...
    const isPolish =
      citizenship?.value === 'Polish' || citizenship?.label?.includes('Polish');
  ...
  const handleFieldChange = (field, value) => {
    const citizenship =
      formRef.current?.citizenship?.value || profileData?.citizenship;
  ...
  const validateForm = () => {
    const formData = new FormData(formRef.current);
    const citizenship =
      selectedCitizenship?.value || selectedCitizenship?.label;
    const errors = {};
    ...
    const isPolish =
      citizenship === 'Polish' || citizenship?.includes('Polish');
    ...
    if (isPolish) {
      ...
      if (!dateOfBirth) {
        errors.dateOfBirth =
          'Date of birth is required for non-Polish citizens';
      }
    }
  ...
  const getFieldVisibility = (field, citizenship) => {
    ...
    return isPolish
      ? visibilityRules.Polish[field]
      : visibilityRules['Non-Polish'][field];
  };
  ...
  const getFieldRequirement = (field, citizenship) => {
    ...
    return isPolish
      ? requirementRules.Polish[field]
      : requirementRules['Non-Polish'][field];
  };
  ...
  const selectStyles = {
    control: (provided, state) => ({
      ...
      border: state.isFocused
        ? '2px solid #3b82f6'
        : formErrors[state.name] && formTouched[state.name]
          ? '1px solid #ef4444'
          : '1px solid #d1d5db',
      borderRadius: '8px',
          ...
          <div className='max-w-4xl mx-auto space-y-6'>
                ...
                <div className='flex items-start space-x-4'>
                  <div className='relative'>
                    ...
                    <div
                      ...
                      onMouseLeave={() =>
                        !showPhotoActions && setShowPhotoOverlay(false)
                      }
            ...
            <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6'>
              ...
              <form
                ...
                className={`${isFormShaking ? 'animate-pulse' : ''}`}
              >
                    ...
                    <div>
                      <label className='block text-sm font-medium text-gray-700 mb-2'>
                        ...
                        <div
                          ...
                          title='Select your citizenship to determine required identification documents'
                        >
                      ...
                      {isEditing ? (
                        ...
                        <p className='text-gray-900'>
                          {selectedCitizenship?.label?.replace(
                            /^[\u{1F1E6}-\u{1F1FF}]{2}\s*/u,
                            ''
                          ) ||
                    ...
                    {getFieldVisibility('pesel', selectedCitizenship) && (
                      <div>
                        <label className='block text-sm font-medium text-gray-700 mb-2'>
                          ...
                          <div
                            className='inline-flex items-center ml-1'
                            title='Polish national identification number (11 digits). Date of birth is automatically calculated from PESEL for Polish citizens.'
                          >
                            <Info className='w-4 h-4 text-gray-400' />
                          </div>
                        </label>
                        {isEditing ? (
                          <input
                            ...
                            onBlur={e =>
                              handleFieldBlur('pesel', e.target.value)
                            }
                            className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                          ...
                          <p className='text-gray-900'>
                            {profileData?.pesel || 'Not provided'}
                          </p>
                        )}
                        {formErrors.pesel && formTouched.pesel && (
                          <p className='mt-1 text-sm text-red-600'>
                        ...
                        {selectedCitizenship?.value === 'Polish' &&
                          ...
                          dateOfBirth && (
                            <p className='mt-1 text-sm text-green-600'>
                              Date of birth calculated from PESEL:{' '}
                    ...
                    {getFieldVisibility(
                      ...
                      <div>
                        <label className='block text-sm font-medium text-gray-700 mb-2'>
                          ...
                          <div
                            className='inline-flex items-center ml-1'
                            title='Required for non-Polish citizens. Please provide your valid passport number.'
                          >
                            <Info className='w-4 h-4 text-gray-400' />
                          </div>
                        </label>
                        {isEditing ? (
                          <input
                            ...
                            onBlur={e =>
                              handleFieldBlur('passportNumber', e.target.value)
                            }
                            className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                              formErrors.passportNumber &&
                              formTouched.passportNumber
                                ? 'border-red-500'
                                : 'border-gray-300'
                            }`}
                    ...
                    {getFieldVisibility('dateOfBirth', selectedCitizenship) && (
                      <div>
                        <label className='block text-sm font-medium text-gray-700 mb-2'>
                          ...
                          <div
                            className='inline-flex items-center ml-1'
                            title='Required for non-Polish citizens. For Polish citizens, this is automatically calculated from PESEL.'
                          >
                            <Info className='w-4 h-4 text-gray-400' />
                          </div>
                        </label>
                        {isEditing ? (
                          <DatePicker
                            ...
                            onBlur={() =>
                              handleFieldBlur('dateOfBirth', dateOfBirth)
                            }
                            className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                    ...
                    {getFieldVisibility(
                      ...
                      <div>
                        <label className='block text-sm font-medium text-gray-700 mb-2'>
                          ...
                          <div
                            className='inline-flex items-center ml-1'
                            title='Polish ID card number (optional)'
                          >
                            <Info className='w-4 h-4 text-gray-400' />
                          </div>
                        </label>
                        {isEditing ? (
                          <input
                            ...
                            name='idCardNumber'
                            defaultValue={
                              profileData?.dowodOsobistyNumber || ''
                            }
                            onChange={e =>
                              handleFieldChange('idCardNumber', e.target.value)
                            }
                            onBlur={e =>
                              handleFieldBlur('idCardNumber', e.target.value)
                            }
                          ...
                          <p className='text-gray-900'>
                            {profileData?.dowodOsobistyNumber || 'Not provided'}
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      {isEditing ? (
                        <input
                          ...
                          onBlur={e =>
                            handleFieldBlur('zipCode', e.target.value)
                          }
                          className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                    ...
                    <div>
                      ...
                      {isEditing ? (
                        <Select
                          ...
                          onBlur={() =>
                            handleFieldBlur('country', selectedCountry?.label)
                          }
                          className='w-full'
                          styles={selectStyles}
                          ...
                          {showOtherProfession && (
                            <div className='mt-3'>
                              ...
                              <input
                                ...
                                onBlur={() =>
                                  handleFieldBlur('profession', otherProfession)
                                }
                                className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                                  formErrors.profession &&
                                  formTouched.profession
                                    ? 'border-red-500'
                                    : 'border-gray-300'
                                }`}
                    ...
                    <div className='bg-white border border-gray-200 rounded-lg p-4'>
                      ...
                      <div className='flex justify-center'>
                        <img
                          ...
                          onError={e => {
                            console.error(
                              'Error loading signature preview:',
                              e
                            );
                            e.target.style.display = 'none';
                          }}
              ...
              <div className='space-y-4'>
                ...
                <div className='flex items-center justify-between'>
                  <div className='flex items-center'>
                    ...
                    <span className='text-gray-900'>
                      {profileData?.phoneNumber || '+48573997766'}
                    </span>
                  </div>
                  <button
                ...
                <div className='flex items-center justify-between'>
                  <div className='flex items-center'>
                    ...
                    <span className='text-gray-900'>
                      {profileData?.email || 'tenant@test.com'}
                    </span>
                  </div>
                  ...
                  <div className='text-center'>
                    {kycFiles.idFront ? (
                      <div className='mb-4'>
                        ...
                        <button
                          onClick={() =>
                            setKycFiles(prev => ({ ...prev, idFront: null }))
                          }
                    ...
                    <input
                      ...
                      accept='.jpg,.jpeg,.png'
                      onChange={e =>
                        handleKycUpload('idFront', e.target.files[0])
                      }
                  ...
                  <div className='text-center'>
                    {kycFiles.idBack ? (
                      <div className='mb-4'>
                        ...
                        <button
                          onClick={() =>
                            setKycFiles(prev => ({ ...prev, idBack: null }))
                          }
                          className='mt-2 text-xs text-red-600 hover:text-red-800'
                        >
                          Remove file
                    ...
                    <input
                      ...
                      accept='.jpg,.jpeg,.png'
                      onChange={e =>
                        handleKycUpload('idBack', e.target.files[0])
                      }
                      className='block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-gray-50 file:text-gray-700 hover:file:bg-gray-100'
                    />
                  </div>
                    ...
                    <div className='space-y-4'>
                      ...
                      <div>
                        <h4 className='text-sm font-medium text-gray-900 mb-2'>
                          GDPR / RODO Consent (English)
                        </h4>
                        <p className='text-sm text-gray-600 leading-relaxed'>
                          I hereby consent to the processing of my personal
                          data, including identity documents, by RentDash for
                          the purpose of verifying my identity in accordance
                          with applicable law. My data will be processed and
                          stored securely for the duration of my account and in
                          compliance with Regulation (EU) 2016/679 (GDPR). I
                          understand that I have the right to access, correct,
                          delete, or limit the processing of my data at any
                          time, as well as the right to lodge a complaint with
                          the relevant supervisory authority. Providing my data
                          is voluntary, however refusal may result in the
                      ...
                      <div>
                        <h4 className='text-sm font-medium text-gray-900 mb-2'>
                          GDPR / RODO Consent (Polski)
                        </h4>
                        <p className='text-sm text-gray-600 leading-relaxed'>
                          Wyrażam zgodę na przetwarzanie moich danych osobowych,
                          w tym dokumentów tożsamości, przez RentDash w celu
                          weryfikacji mojej tożsamości zgodnie z obowiązującymi
                          przepisami prawa. Moje dane będą przetwarzane i
                          przechowywane w sposób bezpieczny przez czas trwania
                          mojego konta, zgodnie z Rozporządzeniem (UE) 2016/679
                          (RODO). Rozumiem, że mam prawo do dostępu,
                          sprostowania, usunięcia lub ograniczenia przetwarzania
                          moich danych w dowolnym momencie, a także prawo do
                          wniesienia skargi do właściwego organu nadzorczego.
                          Podanie danych jest dobrowolne, jednak ich brak może
                ...
                <div className='flex items-start space-x-4'>
                  ...
                  <div className='flex-1'>
                    <h3 className='text-lg font-semibold text-blue-900 mb-2'>
                      Transform Your Account to Business
                    </h3>
                    <p className='text-blue-800 mb-4'>
                      Upgrade to a business account to rent properties under
                      your company name. This creates a professional presence
                      and allows you to add authorized occupants to your rental
                      agreements.
                    </p>
                    ...
                    <div className='mt-2' aria-live='polite'>
                      ...
                      <div className='w-full bg-gray-200 rounded-full h-2'>
                        <div
                          ...
                          style={{
                            width: `${calculatePasswordStrength(passwordData.newPassword).percentage}%`,
                          }}
                        ></div>
                      </div>
                    </div>
