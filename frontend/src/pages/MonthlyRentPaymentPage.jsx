...
const MonthlyRentPaymentPage = () => {
  ...
  const handlePaySecurely = async () => {
    ...
    console.log(
      'Starting monthly rent payment process for:',
    ...
    try {
      ...
      if (response.data.paymentGateway === 'STRIPE') {
        ...
        alert(
          `Stripe payment intent created! Payment ID: ${response.data.paymentIntentId}`
        );
        // You can redirect to Stripe payment form here
      } else if (
        ['PAYU', 'P24', 'TPAY'].includes(response.data.paymentGateway)
      ) {
        ...
        if (response.data.redirectUrl) {
          console.log(
            '🔍 Redirecting to mock payment gateway:',
          ...
          if (mockPaymentWindow) {
            ...
            const checkPaymentCompletion = setInterval(() => {
              if (mockPaymentWindow.closed) {
                ...
                console.log(
                  '🔍 Mock payment window closed, assuming payment completed'
                );
            ...
            setTimeout(() => {
              ...
              console.log(
                '🔍 Mock payment timeout, navigating to success page'
              );
              navigate('/payment-success', {
                state: {
                  paymentType: 'monthly_rent',
            ...
            console.log(
              '🔍 Popup blocked, completing payment directly in backend...'
            );
            ...
            try {
              console.log(
                '🔍 Completing payment directly in backend due to popup blocking...'
              );
              ...
              console.log(
                '✅ Payment completed directly in backend:',
              ...
              console.error(
                '❌ Error completing payment directly:',
                completionError
              );

              // Show alert with manual completion instructions
              alert(
                `Mock ${response.data.paymentGateway} payment created! Please complete your payment at: ${response.data.redirectUrl}`
              );
              ...
              {selectedPaymentMethod === 'PAYU' && (
                <div className='p-6 bg-blue-50 border border-blue-200 rounded-xl'>
                  ...
                  <p className='text-blue-800'>
                    You will be redirected to PayU's secure payment gateway to
                    complete your transaction. PayU supports various payment
                    methods including bank transfers, cards, and digital
                    wallets.
                  </p>
                </div>
              )}
              ...
              {selectedPaymentMethod === 'P24' && (
                <div className='p-6 bg-green-50 border border-green-200 rounded-xl'>
                  ...
                  <p className='text-green-800'>
                    ...
                    Paydoo24 is Poland's leading online payment system
                    supporting bank transfers and cards.
                  </p>
                </div>
              )}
              ...
              <div className='space-y-3'>
                <div className='flex items-center space-x-2'>
                  ...
                  <span className='text-sm text-gray-600'>
                    Member since{' '}
                    {landlordInfo?.createdAt
                      ? new Date(landlordInfo.createdAt).getFullYear()
                      : 'N/A'}
                  </span>
                </div>
        ...
        <div className='mt-8 flex justify-center'>
          ...
          >
            ...
            <span className='text-lg font-semibold'>
              {isProcessing
                ? 'Processing...'
                : `Pay ${formatCurrency(getTotalAmount())} Securely`}
            </span>
          </button>
        </div>
      </div>
      ...
      {showProcessingModal && (
        <div className='fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50'>
          <div className='bg-white rounded-xl p-8 max-w-md w-full mx-4 text-center shadow-2xl'>
            ...
            <div className='space-y-3'>
              <div
                className={`flex items-center space-x-3 ${processingStep >= 1 ? 'text-blue-600' : 'text-gray-400'}`}
              >
                <CheckCircle className='w-5 h-5' />
                <span className='text-sm'>Creating payment intent</span>
              </div>
              <div
                className={`flex items-center space-x-3 ${processingStep >= 2 ? 'text-blue-600' : 'text-gray-400'}`}
              >
                <CheckCircle className='w-5 h-5' />
                <span className='text-sm'>Processing payment</span>
              </div>
              <div
                className={`flex items-center space-x-3 ${processingStep >= 3 ? 'text-blue-600' : 'text-gray-400'}`}
              >
                <CheckCircle className='w-5 h-5' />
                <span className='text-sm'>Confirming payment</span>
              </div>
              <div
                className={`flex items-center space-x-3 ${processingStep >= 4 ? 'text-blue-600' : 'text-gray-400'}`}
              >
                <CheckCircle className='w-5 h-5' />
                <span className='text-sm'>Completing transaction</span>
              </div>
              <div
                className={`flex items-center space-x-3 ${processingStep >= 5 ? 'text-blue-600' : 'text-gray-400'}`}
              >
                <CheckCircle className='w-5 h-5' />
            ...
            {popupBlocked && (
              <div className='mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg'>
                ...
                <p className='text-xs text-yellow-700 mt-1'>
                  Your browser blocked the payment popup. We're completing the
                  payment directly.
                </p>
              </div>
            )}
