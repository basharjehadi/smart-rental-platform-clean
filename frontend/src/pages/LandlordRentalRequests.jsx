...
const LandlordRentalRequests = () => {
  ...
  useEffect(() => {
    ...
    const handleMoveInVerificationUpdate = data => {
      console.log(
        '🏠 Move-in verification update received in rental requests:',
        data
      );
  ...
  const fetchRentalRequests = async () => {
    try {
      ...
      const pendingRequests = (response.data.pending || []).map(req =>
        mapRequest(req, 'pending')
      );
      const offeredRequests = (response.data.offered || []).map(req =>
        mapRequest(req, 'offered')
      );
      const acceptedRequests = (response.data.accepted || []).map(req =>
        mapRequest(req, 'accepted')
      );
      const declinedRequests = (response.data.declined || []).map(req =>
        mapRequest(req, 'declined')
      );
    ...
    try {
      ...
      const mapped = raw.map(req => {
        ...
        const isBusinessRequest =
          req.organization && !req.organization.isPersonal;
        const isGroupRequest =
          req.tenantGroup &&
          req.tenantGroup.members &&
          req.tenantGroup.members.length > 1;
        ...
        return {
          ...
          location: req.location,
          requirements:
            req.additionalRequirements || 'No specific requirements mentioned.',
  ...
  const handleSendOffer = tenant => {
    ...
    console.log(
      'Property available date from database:',
    ...
    if (tenant.propertyMatch.available) {
      try {
        ...
        if (!isNaN(date.getTime())) {
          ...
          console.log(
            'Successfully parsed property available date (local):',
            availableFromDate
          );
        } else {
        ...
        if (tenant.moveInDate) {
          ...
          availableFromDate = `${year}-${month}-${day}`;
          console.log(
            'Using tenant move-in date as fallback:',
            availableFromDate
          );
        } else {
      ...
      if (tenant.moveInDate) {
        ...
        availableFromDate = `${year}-${month}-${day}`;
        console.log(
          'No property date, using tenant move-in date:',
          availableFromDate
        );
      } else {
        // Last fallback to today's date
  ...
  const handlePriceAdjustment = async tenant => {
    try {
      ...
      if (!tenant.propertyMatch || !tenant.propertyMatch.id) {
        const errorMsg =
          'Property match not found. Cannot adjust price without a linked property.';
      ...
      console.log(
        '🔍 Starting price adjustment for property:',
        tenant.propertyMatch.id
      );
      console.log(
        '🔍 Adjusting price from',
      ...
      const updateResponse = await api.put(
        `/properties/${tenant.propertyMatch.id}`,
        {
      ...
      setError(
        `Failed to adjust property price: ${error.message || 'Unknown error occurred'}`
      );
    }
  };
  ...
  const handleSubmitOffer = async () => {
    try {
      ...
      const propertyId =
        selectedTenant.propertyMatch?.id || selectedTenant.matchedProperty?.id;
      if (!propertyId) {
        const errorMsg =
          'Property ID is missing. Cannot create offer without linking to a property.';
      ...
      if (offeredRent > tenantMaxBudget) {
        ...
        console.log(
          `⚠️ Offering rent ${offeredRent} PLN above tenant's max budget ${tenantMaxBudget} PLN (within flexibility)`
        );
      }
      ...
      const requestPayload = {
        ...
        propertyAddress:
          selectedTenant.propertyMatch?.address ||
          selectedTenant.matchedProperty?.address,
        propertyType:
          selectedTenant.propertyMatch?.propertyType ||
          selectedTenant.matchedProperty?.propertyType ||
          'Apartment',
      ...
      console.log(
        '🔍 Making API call to:',
        `/rental-request/${selectedTenant.id}/offer`
      );
      ...
      if (error.response?.data?.code === 'BUDGET_EXCEEDED') {
        ...
        setError(
          `Failed to send offer: ${error.message || 'Unknown error occurred'}`
        );
      }
    } finally {
      setSendingOffer(false);
  ...
  const handleDeclineRequest = async tenant => {
    if (
      window.confirm('Are you sure you want to decline this rental request?')
    ) {
      setDecliningRequest(tenant.id);
      try {
        const response = await api.post(
          `/rental-request/${tenant.id}/decline`,
          {
        ...
        if (response.status === 200) {
          ...
          setRentalRequests(prev =>
            prev.map(req =>
              req.id === tenant.id ? { ...req, status: 'declined' } : req
            )
          );
  ...
  const mockTenants = [
    {
      ...
      location: 'Poznań',
      requirements:
        'Clean, responsible tenant looking for long-term rental. Non-smoker, no pets.',
      propertyMatch: {
    ...
    },
    {
      ...
      location: 'Warszawa',
      requirements:
        'Professional looking for quiet apartment near business district.',
      propertyMatch: {
          ...
          <div className='max-w-7xl mx-auto'>
            <div className='mb-8'>
              ...
              <p className='text-gray-600'>
                Tenant requests matched to your property listings. Review
                profiles and send personalized offers.
              </p>
            </div>
                ...
                <div className='flex items-center'>
                  ...
                  <div>
                    ...
                    <p className='text-sm text-green-700 mt-1'>
                      Rental request declined successfully. The request remains
                      active for other landlords.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className='mb-8 card-modern bg-gradient-to-br from-blue-50 to-indigo-50 p-8'>
                <div className='text-center'>
                  ...
                  <p className='text-lg text-gray-600 mb-6 max-w-2xl mx-auto'>
                    ...
                    from qualified tenants. The more properties you list, the
                    more opportunities you'll have to find great tenants.
                  </p>
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 max-w-4xl mx-auto'>
                    <div className='card-modern p-4'>
                      ...
                      <p className='text-sm text-gray-600'>
                        Maximize your rental income with competitive pricing
                      </p>
                    </div>
                    ...
                    <div className='card-modern p-4'>
                      ...
                      <p className='text-sm text-gray-600'>
                        Connect with verified, responsible tenants
                      </p>
                    </div>
                    ...
                    <div className='card-modern p-4'>
                      ...
                      <p className='text-sm text-gray-600'>
                        Streamlined process from listing to lease
                      </p>
                    </div>
                  </div>
                ...
                <div className='flex'>
                  ...
                  <div className='ml-3'>
                    <p className='text-sm text-blue-800'>
                      Offer sent successfully! The rental request has been
                      removed from your list.
                      {totalRequests > rentalRequests.length && (
                        <span className='block mt-1 text-xs text-blue-600'>
                          You have {totalRequests - rentalRequests.length} other
                          requests with existing offers.
                        </span>
                      )}
                    </p>
                  </div>
              ...
              <div className='flex space-x-2'>
                {['Pending', 'Offered', 'Accepted', 'Declined'].map(tab => {
                  const counts = {
                    ...
                    Offered: rentalRequests.filter(
                      t => (t.status || '').toLowerCase() === 'offered'
                    ).length,
                    Accepted: acceptedRequests.length, // Use accepted requests for this tab
                    Declined: rentalRequests.filter(
              ...
              !error.includes('No properties found') && (
                <div className='card-modern p-12 text-center'>
                  ...
                  <p className='text-gray-500 mb-6'>
                    There are currently no {activeTab.toLowerCase()} requests.
            ...
            <div className='flex items-center justify-between p-6 border-b border-gray-100'>
              <div className='flex items-center space-x-3'>
                ...
                <div>
                  ...
                  <p className='text-sm text-gray-600'>
                    Configure your rental offer terms and conditions for this
                    tenant.
                  </p>
                </div>
              </div>
                ...
                <div className='flex items-center'>
                  ...
                  <div>
                    ...
                    <p className='text-sm text-green-700'>
                      Your offer has been sent to {selectedTenant.name}. They
                      will be notified and can review your terms.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className='p-6 space-y-6'>
              ...
              <div>
                ...
                <p className='text-sm text-gray-600 mb-4'>
                  Tenant's budget: {selectedTenant.budgetRange} • You can adjust
                  your offer within reasonable range.
                </p>
                    ...
                    <div className='ml-3'>
                      ...
                      <div className='mt-2 text-sm text-blue-700'>
                        <p>
                          ...
                          <strong>{selectedTenant.budgetRange}</strong>
                        </p>
                        <p>
                          • Your property is currently listed at:{' '}
                          <strong>{selectedTenant.propertyMatch.rent}</strong>
                        </p>
                        <p>
                          • To send an offer, adjust your price to fit within
                          the tenant's budget
                        </p>
                        <p>
                          • You can offer up to 20% above their max budget if
                          justified
                        </p>
                      </div>
                    </div>
                ...
                {offerData.additionalTerms &&
                  offerData.additionalTerms.includes('Price adjusted') && (
                      ...
                      <div className='flex items-start'>
                        ...
                        <div className='ml-3'>
                          ...
                          <div className='mt-2 text-sm text-green-700'>
                            <p>
                              • Your price has been adjusted from{' '}
                              <strong>
                                {selectedTenant.propertyMatch.rent}
                              </strong>{' '}
                              to <strong>{offerData.monthlyRent} PLN</strong>
                            </p>
                            <p>
                              • This matches the tenant's budget requirements
                            </p>
                            <p>
                              • Security deposit has been calculated as 50% of
                              the adjusted rent
                            </p>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-4'>
                  <div>
                    ...
                    <p className='text-xs text-gray-500 mt-1'>
                      Tenant's max budget:{' '}
                      {selectedTenant?.budgetTo || selectedTenant?.budget} PLN
                    </p>
                  </div>
                  <div>
                    <label className='block text-sm font-medium text-gray-700 mb-2'>
                ...
                {offerData.proRatedCalculation && (
                  <div className='bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4'>
                    <h4 className='text-sm font-medium text-yellow-800 mb-2'>
                      First Month Pro-rating
                    </h4>
                    <div className='text-xs text-yellow-700 space-y-1'>
                      <p>
                        Move-in date:{' '}
                        {new Date(offerData.availableFrom).toLocaleDateString()}
                      </p>
                      <p>
                        Days occupied:{' '}
                        {offerData.proRatedCalculation.daysOccupied} out of{' '}
                        {offerData.proRatedCalculation.daysInMonth}
                      </p>
                      <p>
                        Calculation:{' '}
                        {offerData.proRatedCalculation.daysOccupied}/
                        {offerData.proRatedCalculation.daysInMonth} ×{' '}
                        {offerData.proRatedCalculation.originalRent.toLocaleString(
                          'pl-PL'
                        )}{' '}
                        zł
                      </p>
                      <p className='font-medium'>
                        First month rent:{' '}
                        {offerData.proRatedCalculation.proRatedRent.toFixed(2)}{' '}
                        zł
                      </p>
                    </div>
                  </div>
                ...
                <div className='bg-green-50 border border-green-100 rounded-lg p-4'>
                  <div className='text-center'>
                    ...
                    <p className='text-2xl font-bold text-green-600'>
                      {formatCurrencyDisplay(offerData.totalInitialCost || 0)}{' '}
                      zł
                    </p>
                    <div className='text-sm text-green-600 mt-2 space-y-1'>
                      <p>
                        • First month rent:{' '}
                        {formatCurrencyWithDecimals(offerData.firstMonthRent)}{' '}
                        zł
                      </p>
                      <p>
                        • Security deposit:{' '}
                        {formatCurrencyWithDecimals(offerData.securityDeposit)}{' '}
                        zł
                      </p>
                    </div>
                    <p className='text-sm text-green-600 mt-2'>
                      This is what the tenant will need to pay upfront to secure
                      the property
                    </p>
                  </div>
                </div>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  <div>
                    ...
                    <div>
                      ...
                      {offerData.availableFrom && (
                        <p className='text-xs text-gray-500 mt-1'>
                          Selected: {offerData.availableFrom}
                        </p>
                      )}
                      {selectedTenant.moveInDate && (
              ...
              <div>
                ...
                <p className='text-sm text-gray-600 mt-2'>
                  Include any specific conditions, allowed pets, utility
                  inclusions, or property rules
                </p>
              </div>
            </div>
