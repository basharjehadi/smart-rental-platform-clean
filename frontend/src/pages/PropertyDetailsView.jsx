...
const PropertyDetailsView = () => {
  ...
  const handleAcceptOffer = async () => {
    ...
    try {
      ...
      const response = await api.patch(`/tenant/offer/${offerId}`, {
        status: 'ACCEPTED',
      });
  ...
  const propertyVideo = propertyData.videos
    ? (() => {
        try {
          const parsed =
            typeof propertyData.videos === 'string'
              ? JSON.parse(propertyData.videos)
              : propertyData.videos;
          ...
          console.warn(
            'Failed to parse property videos:',
            propertyData.videos,
            error
          );
          return null;
  ...
  const amenities = propertyData.houseRules
    ? (() => {
        try {
          return typeof propertyData.houseRules === 'string'
            ? JSON.parse(propertyData.houseRules)
  ...
  const propertySpecs = {
    ...
    floor:
      propertyData.floor && propertyData.totalFloors
        ...
        : propertyData.floor
          ? `${propertyData.floor}${getOrdinalSuffix(propertyData.floor)} floor`
          : 'Floor not specified',
      ...
      <div className='flex flex-col'>
            ...
            <div className='flex items-center space-x-4'>
              ...
              <div>
                <h1 className='text-xl font-semibold text-gray-900 line-clamp-1'>
                  {offer.propertyTitle ||
                    propertyData.name ||
                    `${offer.propertyType || 'Apartment'} in ${offer.propertyAddress?.split(',')[2]?.trim() || 'the city'}`}
                </h1>
                <p className='text-sm text-gray-600 line-clamp-1'>
                  {formatAddress(offer.propertyAddress)}
                </p>
              </div>
            </div>
              ...
              {offer?.status === 'PAID' ? (
                ...
                <div className='flex items-center space-x-2 text-sm text-blue-600'>
                  ...
                  <span className='font-medium'>
                    ✅ Offer Accepted - Complete Payment
                  </span>
                </div>
              ) : (
                <>
            ...
            <div className='max-w-7xl mx-auto'>
              <div className='flex items-center justify-center text-green-800'>
                ...
                <span className='text-lg font-semibold'>
                  🎉 Congratulations! You have successfully paid for this
                  property and it is now yours!
                </span>
              </div>
            </div>
          </div>
              ...
              <div className='lg:col-span-2 space-y-6'>
                        ...
                        <div className='flex items-center'>
                          ...
                          <div>
                            <div className='text-sm font-medium text-yellow-800'>
                              Property Information Pending
                            </div>
                            <div className='text-xs text-yellow-600'>
                              This offer is based on your rental request. The
                              landlord will provide detailed property
                              information and photos after you accept this
                              offer.
                            </div>
                          </div>
                        </div>
                          ...
                          <div className='flex items-center space-x-3 p-3 bg-orange-50 rounded-lg'>
                            ...
                            <div>
                              ...
                              <div className='text-xs text-orange-600'>
                                Smoking is permitted in this property
                              </div>
                            </div>
                          </div>
                        )}
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
                      {allAmenities.length > 0 ? (
                        ...
                        <div className='text-sm text-gray-500 italic col-span-full text-center py-4'>
                          {!propertyData.id
                            ? 'Amenities will be provided after property details are available'
                ...
                <div className='card-modern'>
                  <div className='p-6'>
                    ...
                    <p className='text-gray-600 mb-4'>
                      Take a virtual walk-through of this property.
                    </p>
                    <div className='relative bg-gray-100 rounded-lg overflow-hidden aspect-video'>
                      {propertyVideo ? (
                        <video
                          src={getImageUrl(
                            Array.isArray(propertyVideo)
                              ? propertyVideo[0]
                              : propertyVideo
                          )}
                        ...
                        <div className='w-full h-full flex items-center justify-center bg-gray-200'>
                          <div className='text-center'>
                            <div className='text-2xl font-bold text-gray-400 mb-2'>
                              {!propertyData.id
                                ? 'Property Video Pending'
                                : 'No Video Available'}
                            </div>
                            <div className='text-sm text-gray-500'>
                              {!propertyData.id
                                ? 'Virtual tour will be available after property details are provided'
                                : 'Property tour video not uploaded'}
                            </div>
                            <div className='mt-4'>
                ...
                {propertyImages.length > 0 && (
                  <div className='card-modern'>
                    <div className='p-6'>
                      <h2 className='text-lg font-semibold text-gray-900 mb-4'>
                        {propertyImages.length} high-quality images. Click to
                        view full size
                      </h2>
                        ...
                        <span>
                          {formatAddress(
                            offer.propertyAddress ||
                              `${propertyData.address}, ${propertyData.district}, ${propertyData.zipCode}, ${propertyData.city}`
                          )}
                        </span>
                      </div>
                      <div className='flex items-center text-sm text-gray-600'>
                    ...
                    <div className='space-y-3'>
                      <div className='flex justify-between'>
                        ...
                        <span className='text-sm font-semibold'>
                          {formatCurrency(
                            offer.rentalRequest?.budgetTo || offer.rentAmount
                          )}
                        </span>
                      </div>
                      <div className='flex justify-between'>
                  ...
                  <div className='p-6'>
                    ...
                    <div className='space-y-3'>
                      ...
                      <div className='flex justify-between'>
                        ...
                        <span className='font-semibold'>
                          {formatCurrency(
                            offer.depositAmount || offer.rentAmount
                          )}
                        </span>
                      </div>
                      ...
                      {(() => {
                        ...
                        if (moveInDay > 1) {
                          const proRatedRent = Math.round(
                            (offer.rentAmount / daysInMonth) * daysOccupied
                          );
                          return (
                            <div className='flex justify-between'>
                        ...
                        <div className='flex justify-between'>
                          ...
                          <span className='text-green-600 font-bold'>
                            {(() => {
                              ...
                              if (moveInDay > 1) {
                                firstMonthRent = Math.round(
                                  (offer.rentAmount / daysInMonth) *
                                    daysOccupied
                                );
                              }

                              const deposit =
                                offer.depositAmount || offer.rentAmount || 0;
                  ...
                  <div className='card-modern border-green-200 bg-green-50'>
                    <div className='p-6'>
                      ...
                      <p className='text-sm text-green-800'>
                        When you accept this offer, your payment will be held in
                        secure escrow. You'll have 24 hours after check-in to
                        ensure everything matches. Full refund if property
        ...
        {offer?.status === 'PAID' ? (
          ...
          <div className='fixed bottom-0 left-0 right-0 bg-blue-50 border-t border-blue-200 p-4'>
            <div className='flex items-center justify-between max-w-7xl mx-auto'>
              <div className='flex items-center text-blue-800'>
                <Shield className='w-5 h-5 mr-2' />
                <span className='text-sm font-medium'>
                  ✅ Offer Accepted! Complete your payment to secure this
                  property.
                </span>
              </div>
              <div className='flex space-x-3'>
          ...
          <div className='fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4'>
            <div className='flex items-center justify-between max-w-7xl mx-auto'>
              <div className='text-sm text-gray-600'>
                Ready to accept this offer? Your payment will be protected by
                our escrow system.
              </div>
              <div className='flex space-x-3'>
                <button
                  ...
                  {landlordReviews.map(r => (
                    ...
                    >
                      <div className='flex items-start justify-between mb-2'>
                        ...
                        <div className='flex items-center space-x-1'>
                          {[1, 2, 3, 4, 5].map(s => (
                            <Star
                              key={s}
                              className={`w-4 h-4 ${s <= r.rating ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
                            />
                          ))}
                      ...
                      <div className='text-xs text-gray-500'>
                        Posted on {new Date(r.reviewDate).toLocaleDateString()}
                      </div>
                    </div>
                  ))}
                </div>
