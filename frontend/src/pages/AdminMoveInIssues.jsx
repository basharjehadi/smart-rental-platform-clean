            ...
            <div key={o.id} className="p-4 bg-white rounded border">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm text-gray-600">Reason: {o.cancellationReason || '—'}</div>
                  {Array.isArray(o.cancellationEvidence) && o.cancellationEvidence.length > 0 && (
                    <div className="mt-2 text-xs text-gray-700">
                      Evidence:
                      <ul className="list-disc ml-5">
                        {o.cancellationEvidence.map((p, idx) => (
                          <li key={idx}><a className="text-blue-600 hover:underline" href={`http://localhost:3001${p}`} target="_blank" rel="noreferrer">{p}</a></li>
                        ))}
                      </ul>
                    </div>
                  )}
                ...
                <div className="flex items-center space-x-2">
                  <button className="px-3 py-1 rounded bg-green-600 text-white" onClick={() => takeAction(o.id, 'approve')}>Approve</button>
                  <button className="px-3 py-1 rounded bg-red-600 text-white" onClick={() => takeAction(o.id, 'reject')}>Reject</button>
                </div>
              </div>
            </div>
          ))}
