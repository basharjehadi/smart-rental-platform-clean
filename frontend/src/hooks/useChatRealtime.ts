...
export const useChat = (): UseChatReturn => {
  ...
  const loadMessages = useCallback(async (conversationId: string, page = 1) => {
    ...
    try {
      const limit = 50;
      const res = await fetch(`${API_BASE}/messaging/conversations/${conversationId}/messages?page=${page}&limit=${limit}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      try {
        const unreadIds = (Array.isArray(data) ? data : []).filter(m => m.senderId && m.senderId !== (activeConversation as any)?.currentUserId && !m.isRead).map(m => m.id);
        if (unreadIds.length > 0) {
          await fetch(`${API_BASE}/messaging/conversations/${conversationId}/messages/read`, {
            method: 'PUT',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (msg: Message & { conversationId?: string }) => {
      ...
      if (currentActive && msg && msg.conversationId === currentActive.id) {
        ...
        console.log('🔄 Message not for active conversation, refreshing conversations');
