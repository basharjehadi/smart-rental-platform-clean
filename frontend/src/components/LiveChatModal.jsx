...
const LiveChatModal = ({ isOpen, onClose }) => {
      ...
      setMessages([
        {
          id: 'error',
          message:
            "Sorry, we couldn't start the chat session. Please try again or submit a support ticket.",
          senderId: 'system',
          timestamp: new Date(),
          isRead: true,
        },
  ...
  const sendMessage = async e => {
    ...
    try {
      const response = await api.post(
        `/support/chat/${chatSession.id}/message`,
        {
  ...
  const uploadAttachments = async files => {
    ...
    const res = await api.post(
      `/support/chat/${chatSession.id}/attachments`,
      form,
      {
      ...
      <div className='bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col'>
        ...
        <div className='px-4 py-3 bg-gray-50 border-t border-gray-200 rounded-b-lg'>
          <p className='text-xs text-gray-600 text-center'>
            Type your message to start chatting with support. This session will
