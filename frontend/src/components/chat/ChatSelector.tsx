...
const ChatSelector: React.FC<ChatSelectorProps> = ({
  ...
  const handleStartChat = async (targetPropertyId: string) => {
    try {
      console.log(
        '🔍 handleStartChat called with propertyId:',
        targetPropertyId
      );
      setLoading(true);
      setError(null);
      ...
      if (response.data.success) {
        ...
        console.log(
          '🔍 Navigating to:',
          `/messaging/${response.data.conversationId}`
        );
          ...
          {loading ? (
            ...
            <div className='text-center py-8'>
              ...
              <h3 className='text-lg font-medium text-gray-900 mb-2'>
                No properties available for chat
              </h3>
              <p className='text-gray-600'>
                You need to have paid for a property to start chatting with the
                landlord.
              </p>
            </div>
          ) : (
