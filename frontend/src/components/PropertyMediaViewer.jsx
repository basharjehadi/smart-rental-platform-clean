...
const PropertyMediaViewer = ({
  ...
  const amenities =
    typeof propertyAmenities === 'string'
      ? propertyAmenities
        ? (() => {
            try {
              return typeof propertyAmenities === 'string'
  ...
  const absoluteVideoUrl = propertyVideo
    ? propertyVideo.startsWith('http')
      ? propertyVideo
      : propertyVideo.startsWith('/')
        ? `http://localhost:3001${propertyVideo}`
        : `http://localhost:3001/${propertyVideo}`
    : null;
      ...
      {absoluteImageUrls.length > 0 && (
        <div className='mb-4'>
          <h5 className='text-sm font-medium text-gray-700 mb-2'>
            Property Images ({absoluteImageUrls.length})
          </h5>
