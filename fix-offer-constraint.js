...
const fixConstraintAndCreateOffers = async () => {
  try {
    console.log('🔧 Fixing database constraint and creating test offers...\n');

    // First, let's try to drop the unique constraint manually
    try {
      await prisma.$executeRaw`DROP INDEX IF EXISTS offers_rentalRequestId_key`;
      console.log('✅ Removed unique constraint from offers table');
    } catch (error) {
